...
public class BisectrixComponent implements PlanimetriaComponent {
    @Override
    ...
    public UnaryOperator<String> parseExpression() {
        return expr -> {
            ...
            String result = showBisectrix(x, y, point).toString();
            return result.substring(1, result.length() - 1);
        };
    }

    private F showBisectrix(double[] x, double[] y, double[] pointFrom) {
        //1 line
        ...
        F t3 = tablePlot(ff3);
        // bisectrix
        double[][] bisectrix = getBisectrixCoords(x, y, pointFrom);
        F fx4 = vector(bisectrix[0][0], bisectrix[1][0]);
        F fy4 = vector(bisectrix[0][1], bisectrix[1][1]);
    ...
    private static double[][] getBisectrixCoords(double[] x, double[] y, double[] pointFrom) {
...
//        [ [x1,y1],[x2,y2] ]
        ...
        double x1 = segment[0][0],
                ...
                y2 = segment[1][1],
                y3 = pointFrom[1],
                a = Math.sqrt(Math.pow((x1 - x3), 2) + Math.pow((y1 - y3), 2)),
