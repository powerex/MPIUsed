...
public class Parser {
     ...
     */
    public com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode parse(String expression) {
     ...
     * expression tree made out of fviz.lexerParser.ExpressionNode objects
     */
    public com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode parse(LinkedList<Token> tokens) {
        // implementing a recursive descent parser
        this.tokens = (LinkedList<Token>) tokens.clone();
        lookahead = this.tokens.getFirst();

        // top level non-terminal is expression
        com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode expr = expression();
    ...
    private ExpressionNode signedTerm() {
        ...
        if (lookahead.token == Token.PLUSMINUS) {
            boolean positive = lookahead.sequence.equals("+");
            nextToken();
            com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode t = term();
            if (positive)
                return t;
            else
    ...
    private ExpressionNode signedFactor() {
        ...
        if (lookahead.token == Token.PLUSMINUS) {
            boolean positive = lookahead.sequence.equals("+");
            nextToken();
            com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode t = factor();
            if (positive)
                return t;
            else
                return new AdditionExpressionNode(t, false);
    ...
    private ExpressionNode factorOp(ExpressionNode expr) {
        ...
        if (lookahead.token == Token.RAISED) {
            nextToken();
            com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode exponent = signedFactor();
    ...
    private ExpressionNode argument() {
        ...
        if (lookahead.token == Token.FUNCTION) {
            ...
            nextToken();
            com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode expr = argument();
    ...
    private ExpressionNode value() {
        ...
        if (lookahead.token == Token.NUMBER) {
            com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode expr = new ConstantExpressionNode(lookahead.sequence);
            nextToken();
            return expr;
        }
        ...
        if (lookahead.token == Token.VARIABLE) {
            com.mathpar.Graphic2D.tangent.lexerParser.ExpressionNode expr = new VariableExpressionNode(lookahead.sequence);
            nextToken();
            return expr;
        }
