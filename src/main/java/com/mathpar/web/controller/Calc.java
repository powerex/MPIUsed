...
public class Calc {
    ...
    @RequestMapping(value = "/api-app/check", method = RequestMethod.POST)
    public IMathparResponse check(@RequestBody MathparAppCheckRequest checkRequest,
                                  @PageParam Page page) throws IOException {
        System.out.println(checkRequest.userAnswer);
        System.out.println(checkRequest.dbSolutionAnswer);
        CheckResult checkResult = page.check(checkRequest.userAnswer, checkRequest.dbSolutionAnswer);
        return MathparResponse.ok(checkResult.getDescription(), checkResult.getDescription());
    }
    ...
    private static String memoryInfo() {
        ...
        long freeMem = (maxMemory - RUNTIME.totalMemory() + RUNTIME.freeMemory())
                / MEGABYTES;
        String ss= freeMem + " / " + (maxMemory / MEGABYTES) + "MB";
        return ss;
    }
}
