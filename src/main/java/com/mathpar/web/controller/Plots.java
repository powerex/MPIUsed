...
import com.mathpar.web.executor.PlotCollection3DTimeoutRunner;
...
public class Plots {
    ...
    @RequestMapping(value = "/api/showplots3d", method = RequestMethod.POST)
    public ResponseEntity<Plots3DCollection> getMeshFor3dPlotCollection(
            @RequestBody MathparRequest mpReq, @PageParam Page page) {
        final String task = mpReq.getTask();
        if (!task.contains(F.FUNC_NAMES[F.SHOW_PLOTS_3D])) {
            LOG.warn("Input task '{}' doesn't contain function {}.",
                    task, F.FUNC_NAMES[F.SHOW_PLOTS_3D]);
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
        final Plots3DCollection geometry = new PlotCollection3DTimeoutRunner()
                .run(page, task, mpReq.getSectionId());
        if (geometry == null) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
