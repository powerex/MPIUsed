...
//    }
     ...
     * @param di   - array of the diagonal elements
     * @param ring - Ring
     */
    public MatrixS(int size,  Element[]di, Ring ring) {
        M= new Element [size][]; col= new int [size][];
        for (int i = 0; i < di.length; i++) {
           if (!di[i].isZero(ring)){
              M[i]=new Element[]{di[i]}; col[i]=new int[]{i}; }}
        this.size = size;
        this.colNumb = di.length;
    }
...
/** MatrixS with one first row VectorS d  
*   @param d   (VectorS) - first row]
*/
    public MatrixS(VectorS d, Ring r) {
     ...
     /** Constructor for matrix of vector of vectors as a dense matrix
     * @param s   VectorS[VectorS[]]
     */
    public MatrixS(VectorS[] s, int sazhneva, Ring ring) {
...
/**  Возвращает указанную строку матрицы квадратной в плотном формате,
 *   добавля, где надо, нули
     * @param i номер строки
     * @return элемент матрицы в позиции (i,j)
     */
      ...
      /**  Getting row block started from row _first_ with total number of rows equals _numb_.
       *   All indecex are copied to new place.
     * @param first -- index of the first row
     * @param numb  -- total number of getting rows
     * @return matrixS obtained of this  numb rows
     */
      public  MatrixS getRowBlock(int first, int numb) {
        int len = M.length;
        Element[][] MM=new Element[numb][0];
        int[][] cc=new int[numb][0];
        int s=0;
        for (int j = first; j < first+numb; j++) {int[] c=new int[col[j].length];
            System.arraycopy(col[j], 0, c, 0, c.length);
            MM[s]= M[j]; cc[s++]= c;              
        } 
         MatrixS res=new MatrixS(numb, colNumb, MM, cc);
          System.out.println(numb+"     getRowBlock==="+res);
         return res;
    }
    /**
     *  Get column of MatrixS as a dense array
    ...
    public MatrixS getSubMatrix(int startRow, int endRow, int startCol, int endCol) {
        ...
        return scalarMatrix( n,   n,  a,   ring);
    }


    /** Построение иагональной матрицы   c элементоми a[] на диагонали.
     * @param a[]  числа на диагонали
     */
    public static MatrixS diagonalMatrix(  Element[] a, Ring ring) { int n=a.length;
        if (n==0) {
            return zeroMatrix(0);
        }
        ...
        /** Построение иагональной матрицы c элементоми a[] на второй диагонали.
     * @param a[]  числа на  второй  диагонали
     */
    public static MatrixS diagonalMatrixSecond(  Element[] a, Ring ring) { int n=a.length;
        if (n==0) { return zeroMatrix(0);}
        Element[][] r = new Element[n][1];
        int[][] c = new int[n][1];
        for (int i = 0; i < n; i++) {
            r[i][0] = a[i];
...
//-------
 ...
 //    System.out.println("size="+size+"  "+ colNumb+"  "+M.length+this);
        int n = Math.max(size, colNumb);
        int hb = Integer.highestOneBit(n);
  //      System.out.println("hb="+hb+"   n="+n+"  col.len= "+col.length);
        if (hb != n) {hb <<= 1;}
        Element[][] MM=new Element[n][0];int[][] cc=new int[n][0];
        for (int i = 0; i < col.length; i++) {MM[i]=M[i];cc[i]=col[i];}
...
//  ssh 22  ftp 21 http 80 - apach jsp//
    ...
    public MatrixS ES_min_dI(Element d, int[] Ei, int[] Ej,  Ring r) {
        if ((Ei.length == 0) || (isZero(r))) {
            return scalarMatrix(size, d.negate(r), r);
        }
        if (size == 1) {
            return zeroMatrix(size);
        }
...
//    }
    ...
    public VectorS[] solve(int m, Ring ring) {Integer[] sizeNullSp=new Integer[1];
       ...
       int[][] ccol=kernel.col;
       int i=ccol.length-1;
       for (; i >= 0 ; i--) {  int j=ccol[i].length;
         if (j>0){j--; if(det==null){det=kernel.M[i][j].negate(ring); } // сохранили определитель
        ...
        if((type ==Ring.Q)||(type<=Ring.Z)||((type>=Ring.CZ64)&&(type<=Ring.CZ) )||(type ==Ring.CQ)   ) {
            vv[sNS-1]=vv[sNS-1].divideToFraction(det, ring);
        } else {
...
//        return kernel(m, null, ring);}
    ...
    public MatrixS kernel(int m, Integer[] sizeOfNullSpase, Ring ring) {
        ...
        if(sizeOfNullSpase!=null) {
            sizeOfNullSpase[0]=m-EiEj[0].length;
        }
        int i = EiEj[0][0];
        int j = EiEj[1][0];
        int p = 0;
...
public static MatrixS Eij2MatrixS_Z(int[] Ei, int[] Ej, int N) {
    ...
    for(int i=0; i<Ei.length;i++) {
        res.putElement(NumberZ.ONE, Ei[i], Ej[i]);
    }
    return res;
}
//   /**
...
//    }
  ...
  public Object clone(){ int[][] c=new int[col.length][];
   ...
   public   MatrixS permutationOfColumns(int[] Ec){
      int n=(Ec.length>>1);  if(n==0) {
           return this;  // n -- половинка длины Ec
       }
      int[] colNumbs= new int[colNumb];
      if (MaxPositiveNumber< colNumb) {
          newMaxPositiveNumbers(4*colNumb);
      }
   ...
   /**
    * 
    * @param d -- array of MatrixS which can be put at diagonal
    * @param r -  rows number at i-th matrix (i=0,1,..,r.length-1
    * @return  - BlockDiagonal MatrixS with r[0]+r[1]+..+r[r.length-1]
    */
  public static MatrixS makeBlockDiagonalMatrixS(MatrixS[] d, int[] r ){
      int m=Math.min(r.length,d.length); int rowsN=0;
      for (int i = 0; i < m; i++) {
          rowsN+=r[i];
      }
      Element[][] MM=new Element[rowsN][0]; int[][] c=new int[rowsN][0];
      int s=0; //row index
      int shift=0; // shift
      int colNumbN=0;
      for (int k = 0; k < m; k++) {
        ...
        for (int i = 0; i < r[k]; i++) {  
          ...
          {c[s]=d[k].col[i]; } 
              ...
              int[] ci=new int[d[k].col[i].length]; 
          c[s]=ci;
             for (int j = 0; j < d[k].col[i].length; j++) {
                 ci[j]=d[k].col[i][j]+shift;
             }
          }  MM[s++]=d[k].M[i]; 
        }shift+=r[k]; colNumbN=Math.max(colNumbN, d[k].colNumb); }
      }  
      System.out.println("rowsN--colNumbN --------- c.lenth"+  rowsN+colNumbN+c.length);
       return new MatrixS( rowsN,colNumbN, MM,c);
  }
  
     /** Concatenation of two MatrixS: First=this and Second=b
      *  
      * @param b - second matrix
      * @param shiftb - the number of columns in the first matrix
      * @return concatenation of two matrices
      */
  public MatrixS concatenationLR(MatrixS b, int shiftb){ if(shiftb==0) {
      return b;
  }
      int m=Math.max(M.length,b.M.length); int min=Math.min(M.length,b.M.length); 
      Element[][] MM=new Element[m][]; int[][] c=new int[m][];
      for (int i = 0; i < min; i++) {
          c[i]=Array.concatenation(col[i], b.col[i]);
          for (int j = col[i].length; j < c[i].length; j++) {c[i][j]+=shiftb;}
          MM[i]=Array.concatenation(M[i], b.M[i]);
      }
      if(M.length>min) {
          for (int i = min; i < m; i++) {MM[i]=M[i]; c[i]=col[i];}
      }
      if(b.M.length>min) {
          for (int i = min; i < m; i++) {
            int[] cc=new int[b.col.length]; System.arraycopy(b.col[i], 0, cc, 0, b.col.length);
            for (int j = 0; j < cc.length; j++) {cc[j]+=shiftb;}
            c[i]=cc; MM[i]=b.M[i]; }
      }
      int sizeN=Math.max(size,b.size); int colNumbN=shiftb+b.colNumb;           
       return new MatrixS( sizeN,colNumbN, MM,c);
  }
       /** Concatenation of two MatrixS: Up=this and Down=b
      *  
      * @param b - Down matrix
      * @param shiftb - the number of rows in the first matrix
      * @return concatenation of two matrices
      */
  public MatrixS concatenationUD(MatrixS b, int shiftb){ if(shiftb==0) {
      return b;
  }
      System.out.println(shiftb+"   b="+b+"   this=" +this);
      int m=Math.max(M.length,b.M.length);  
      Element[][] MM=new Element[shiftb+b.col.length][0]; 
      int[][]      c=new int[shiftb+b.col.length][0];
      for (int i = 0; i < shiftb; i++) {c[i]= col[i]; MM[i]= M[i];  System.out.println("AAA="+Array.toString(M[i] )+ Array.toString(col[i]));}
      for (int i = 0; i < b.col.length; i++) {c[i+shiftb]= b.col[i]; MM[i+shiftb]= b.M[i]; System.out.println("AAA11="+Array.toString(b.M[i] )+Array.toString(b.col[i]));}      
      int colNumbN=Math.max(colNumb,b.colNumb); int sizeN=shiftb+b.size;           
      MatrixS res=new MatrixS( sizeN,colNumbN, MM,c);
      System.out.println("res="+res);
      return res;
  }
  
     /**
    *  Delete the Block Diagonal from this MatrixS which has more then
    *       r[0]+r[1]+..+r[r.length-1] rows
    * @param r -  rows number at i-th block (i=0,1,..,r.length-1
    * @return  - matrixS without diagonal blocks with 
    *  r[1]+..+r[r.length-1] number of rows
    */
  public MatrixS deleteBlockDiagonalMatrixS(int[] r ){
      int rowsN=0;
      for (int i = 0; i < r.length; i++) {
          rowsN+=r[i];
      }
     // int maxblocksize= Array.max(r);
      int[] delInd=new int[this.colNumb];
      Element[][] MM=new Element[rowsN][0]; int[][] c=new int[rowsN][0];
      int f=0;    //row index first
      int l=0; //row index last
      for (int k = 0; k < r.length; k++) {l+=r[k]; // f we append in the end of this block
        for (int i = f; i < l; i++) {int s=0;  int[] ci=col[i]; Element[] Mi=M[i];
             for (int j = 0; j <  ci.length; j++) {
                 if ((ci[j]>=f)&&(ci[j]<l)) {
                     delInd[s++] = j;
                 }
             ...
             Element[] newR=Mi; int[] newC=col[i];
             if(s!=0){int len=ci.length-s; newR=new Element[len]; newC=new int[len];
                 System.out.println("len"+len);
               System.arraycopy(ci, 0, newC, 0, len);
               System.arraycopy(Mi, 0, newR, 0, len);int j=0;
               int sd=delInd[0], m=0;
               while (sd<len){                
                     while((m<s-1) &&(delInd[m]+1==delInd[m+1])) {
                         m++;
                     }
                     int last=  (m==s-1)? ci.length: delInd[m+1];
                     System.out.println("last="+last+"  i"+i+ "  "+ ci[delInd[m]+1]);
                     for (int n = delInd[m]+1; n < last; n++) 
                                {newC[sd]=ci[n]; newR[sd++]=Mi[n];}}
            }
           MM[i]=newR; c[i]=newC;
        }   f+=r[k];  
        } 
       return new MatrixS( rowsN,this.colNumb, MM,c);
  }
  ...
  public static int[] multPermutations(int[] e0Inp, int[] e1Inp, int N ){
      ...
      if (l0==0) {
       ...
       if (MaxPositiveNumber< N) {
           newMaxPositiveNumbers(4*N);
       }
...
//   }
  ...
  /** make Block-Diagonal Matrix ??? SEE TUGAREVA
   * 
   * @param Kl
   * @param rowNumb -- THE NUMBER OF ROWS IN NEW MATRIX
   * @return 
   */  
  public static MatrixS makeBlockDiagonalMatrix(MatrixS[][] Kl, int rowNumb) {
        ...
        while (i < rowNumb) { 
           ...
           // System.out.println("Kl" + i + "=" + Array.toString());
            ...
            if (Kl[kx].length == ky) {ky = 0; kx++; 
                if (Kl.length == kx) {System.out.println("tyui");
                    break;
                }
            }
...
/**
   ...
   public MatrixS AmulE(int  I, int J, int L){
     if (L==0) {
         return MatrixS.zeroMatrix(size);
     }
    ...
    * Для L12: B=L;
     ...
     * @return  result of multiplication = this*E
     */
  public MatrixS EmulA(int i, int j, int l ){ if (M.length==0) {
      return this;
  }
     if  (l==0) {
         return MatrixS.zeroMatrix(size);
  ...
  public MatrixS insertRowsIn(MatrixS B,  int firstInThis, int firstInB, int number ){
    ...
    /**
   * To move rows of this matrix to this matrix.
...
//    }
 ...
 public static Element[] emptyElementRow=new Element[0];
     /**
   * To move columns of of this matrix to this matrix.
  ...
  public MatrixS cloneWithMoveColumns(int fromCol, int toCol, int number ){ int shift=toCol-fromCol;
    ...
    for (int j = 0; j<col.length; j++) { int[] cj=col[j].clone(); c[j]=cj;
       for (int i = 0; i < cj.length; i++){if((cj[i]>=fromCol)&&(cj[i]<=maxCol)) {
           cj[i]+=shift;
       }
       }}
      ...
      if (nvars>0){
          ...
          for (int i=0; i<this.size; i++){
              ...
              for (int j=0; j<this.M[i].length; j++){
                  ...
                  if(len[i]<p.coeffs.length) {len[i]=p.coeffs.length; if(len[i]>len_max) {
                      len_max=len[i];
                  }
                  }
                  Element max=r.numberZERO;
                  for (int k=0; k<p.coeffs.length; k++){
                      if(!max.compareTo(p.coeffs[k], 2, r)) {
                          max=p.coeffs[k];
                      }
                     // if (k*nvars<p.powers.length){
                        int step=k*nvars+nvars;
                        if(p.powers.length<step) {
                            step=p.powers.length;
                        }
...
//    }
    ...
    /** Вычисление одного рационального решения x для системы неравенств Ax>=b
     * присоединенной матрицы, эшелонной матрицы и матрицы
     *  ведущих столбцов
     *
     * @param A   матрица  
     * @return b - вектор элементов в правой части
     */
    public VectorS[] oneSolutionOfInequalities(int m, Ring ring) {
        VectorS[] res =solve(m, ring);
        
     /*   MatrixS gg=this; Elemen t one=ring.numberONE;
        int[][] EiEj= new int[2][0];
        AdjMatrixS x = new AdjMatrixS(expandToPow2with0(), one, ring);
        EiEj[0] = x.Ei;
        EiEj[1] = x.Ej;*/
        return res;
    ...
    public static void main(String[] args) {
        ...
        // MatrixS adjointDet(Element[] oneDet, Ring ring)
...
//ilch        ElementBufferAllocator buffers=new ElementBufferAllocator(2, ring);
//ilch  int N=500,M=500,density=10000;   
            //    int N=64,M=64,density=5; 
                   int N=3,M= 4,density=100; 
        MatrixS a=new MatrixS(new long[][]{{3,0,5,1},{1,0,1,3},{1,0,1,1} }, ring);
        // (N, M, density, n=mmmew int[]{5}, new Random(), ring.numberONE, ring);
   // MatrixS b=new MatrixS(N, M, density, new int[]{300}, new Random(), ring.numberONE, ring);
        System.out.println("Mat="+a.toString(ring));
        //System.out.println(b.toString(ring));
        long startMTtime=System.currentTimeMillis();
        Element[] det0=new Element[]{ring.numberONE};
    MatrixS  adj= a.adjointDet(det0 ,  ring) ; int mm=4;
    VectorS[] solv= a.solve(mm,  ring);
    int lenS=solv.length; for (int i = 0; i < lenS; i++) { Element[] temp=new Element[mm];
            for (int j=0; j< mm-1; j++) {temp[j]=solv[i].V[j];} temp[mm-1]=ring.numberMINUS_ONE;   //det0[0].negate(ring);
                VectorS tempV= new VectorS(temp);

            System.out.println("V="+solv[i]+"  a*sol="+a.multiplyByColumn(tempV,ring));
        }
        System.out.printf("adj=%s  det=%s%n", adj, det0[0]);
//ilch     MatrixS c=a.multiplyMT(b,buffers.getMaximalUsedThreads(), buffers);
        System.out.println("MT runtime="+(System.currentTimeMillis()-startMTtime));
        long startSeqtime=System.currentTimeMillis();
//ilch    MatrixS trueC=a.multiply(b, ring);
        ...
        //  Temp proc - must delete =======================================
    
  public MatrixS EmulA___(NewLDU m ){int i=m.I; int j=m.J; int l=m.rank;
  return  EmulA(i,j,l);}
  public MatrixS ETmulA(NewLDU m ){int i=m.I; int j=m.J; int l=m.rank; return  EmulA(j,i,l);}
  public MatrixS ImulA(NewLDU m ){int i=m.I; int l=m.rank; return  EmulA(i,i,l);}
  public MatrixS barImulA(NewLDU m ){int i=m.I; int l=m.rank; int N=M.length;
    if (l>=N) {
        return MatrixS.zeroMatrix(size);
    }
    if(i==0) {
     ...
     *  @return Eij^T * S - d * I типа MatrixS
     */
    public MatrixS ES_min_dI(Element d, NewLDU m, Ring r) {
        if ((m.rank == 0) || (isZero(r))) {
            return scalarMatrix(size, d.negate(r), r);
        }
        if (size == 1) {
