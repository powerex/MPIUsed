...
//**********Принимаем блоки матриц А и В и продолжаем разбиение***********************             
            ...
            if (levl != 0) {
                ...
                for (int i = 0; i < levl; i++) {
                    ...
                    t.send(fmAA[2], MPI.COMM_WORLD.Rank() + 1 * procsize, 200000 + i);
                    t.send(fmAA[3], MPI.COMM_WORLD.Rank() + 1 * procsize, 200000 + i);
                    t.send(fmBB[0], MPI.COMM_WORLD.Rank() + 1 * procsize, 200000 + i);

                    t.send(fmAA[0], MPI.COMM_WORLD.Rank() + 2 * procsize, 300000 + i);
                    t.send(fmBB[1], MPI.COMM_WORLD.Rank() + 2 * procsize, 300000 + i);
                    t.send(fmBB[3], MPI.COMM_WORLD.Rank() + 2 * procsize, 300000 + i);

                    t.send(fmAA[3], MPI.COMM_WORLD.Rank() + 3 * procsize, 400000 + i);
                    t.send(fmBB[2], MPI.COMM_WORLD.Rank() + 3 * procsize, 400000 + i);
                    t.send(fmBB[0], MPI.COMM_WORLD.Rank() + 3 * procsize, 400000 + i);

                    t.send(fmAA[0], MPI.COMM_WORLD.Rank() + 4 * procsize, 500000 + i);
                    t.send(fmAA[1], MPI.COMM_WORLD.Rank() + 4 * procsize, 500000 + i);
                    t.send(fmBB[3], MPI.COMM_WORLD.Rank() + 4 * procsize, 500000 + i);

                    t.send(fmAA[2], MPI.COMM_WORLD.Rank() + 5 * procsize, 600000 + i);
                    t.send(fmAA[0], MPI.COMM_WORLD.Rank() + 5 * procsize, 600000 + i);
                    t.send(fmBB[0], MPI.COMM_WORLD.Rank() + 5 * procsize, 600000 + i);
                    t.send(fmBB[1], MPI.COMM_WORLD.Rank() + 5 * procsize, 600000 + i);

                    t.send(fmAA[1], MPI.COMM_WORLD.Rank() + 6 * procsize, 700000 + i);
                    t.send(fmAA[3], MPI.COMM_WORLD.Rank() + 6 * procsize, 700000 + i);
                    t.send(fmBB[2], MPI.COMM_WORLD.Rank() + 6 * procsize, 700000 + i);
                    t.send(fmBB[3], MPI.COMM_WORLD.Rank() + 6 * procsize, 700000 + i);
                }
                //** II этап********************Умножение собственных блоков******************************
                res[0] = fmA.multCU(fmB, getNextUnusedFileName(rank));
//*****III этап *****************Сборка окончательного результата**************************            
...
//**********приемка блоков от дочерних процессоров*********************** 
                    if (PRINT_LOGS){
                        printLog(rank, "recv dBlocks start... startRank="+(MPI.COMM_WORLD.Rank() + 1 * procsize)
                        +" end rank="+(MPI.COMM_WORLD.Rank() + 6 * procsize + myRank));
                    }
