          ...
          //  printLog(rank,"start recv blocks...");
            ...
            if (levl != 0) {
                ...
                for (int i = 0; i < levl; i++) {
                    ...
                    fmA = fmAA[0];                 
                    fmB = fmBB[0];
                    t.send(fmAA[1], MPI.COMM_WORLD.Rank() + 1 * procsize, i);
                    t.send(fmBB[2], MPI.COMM_WORLD.Rank() + 1 * procsize, i);
                    t.send(fmAA[0], MPI.COMM_WORLD.Rank() + 2 * procsize, i);
                    t.send(fmBB[1], MPI.COMM_WORLD.Rank() + 2 * procsize, i);
                    t.send(fmAA[1], MPI.COMM_WORLD.Rank() + 3 * procsize, i);
                    t.send(fmBB[3], MPI.COMM_WORLD.Rank() + 3 * procsize, i);
                    t.send(fmAA[2], MPI.COMM_WORLD.Rank() + 4 * procsize, i);
                    t.send(fmBB[0], MPI.COMM_WORLD.Rank() + 4 * procsize, i);
                    t.send(fmAA[3], MPI.COMM_WORLD.Rank() + 5 * procsize, i);
                    t.send(fmBB[2], MPI.COMM_WORLD.Rank() + 5 * procsize, i);
                    t.send(fmAA[2], MPI.COMM_WORLD.Rank() + 6 * procsize, i);
                    t.send(fmBB[1], MPI.COMM_WORLD.Rank() + 6 * procsize, i);
                    t.send(fmAA[3], MPI.COMM_WORLD.Rank() + 7 * procsize, i);
                    t.send(fmBB[3], MPI.COMM_WORLD.Rank() + 7 * procsize, i);
                }
 //** II этап********************Умножение собственных блоков******************************
                res[0] = fmA.multCU(fmB, getNextUnusedFileName(rank));              
//*****III этап *****************Сборка окончательного результата**************************            
                ...
                for (int i = 0; i <levl ; i++) {
//**********приемка блоков от дочерних процессоров***********************            
                    res[1] = t.recv(MPI.COMM_WORLD.Rank() +1 * procsize, i);            
                    res[2] = t.recv(MPI.COMM_WORLD.Rank() +2 * procsize, i);            
                    res[3] = t.recv(MPI.COMM_WORLD.Rank() +3 * procsize, i);             
                    res[4] = t.recv(MPI.COMM_WORLD.Rank() +4 * procsize, i);              
                    res[5] = t.recv(MPI.COMM_WORLD.Rank() +5 * procsize, i);              
                    res[6] = t.recv(MPI.COMM_WORLD.Rank() +6 * procsize, i);               
                    res[7] = t.recv(MPI.COMM_WORLD.Rank() +7 * procsize, i);
                    procsize = procsize * 8;
//***********формирование промежуточных матриц****************************   
                    sum[0] = res[0].add(res[1], getNextUnusedFileName(rank));
                    sum[1] = res[2].add(res[3], getNextUnusedFileName(rank));
