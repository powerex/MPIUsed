...
import com.mathpar.splines.spline1D.approxType;
import com.mathpar.splines.spline2D;

/**
 *
 * @author yuri
 */
public class GM_Spline2D extends GM_Figure {

    public GM_Spline2D(GM_Figure Parent, byte type, GM_Color color, double[][] f, approxType aType) {
        super(Parent, type, color);
        spline2D sp2d = new spline2D(f, aType);
        int maxx = f.length - 1;
        int maxy = f[0].length - 1;
        int M = 1+maxy * 30;
        int N = 1+maxx * 30;
