...
public class Dcm2Txt implements DicomInputHandler {
    ...
    public static void main(String[] args) {
        ...
        File ifile = new File("dcm/2-02.dcm");
        try {
            dcm2txt.dump(ifile);
        } catch (IOException e) {
    ...
    public  SortedMap<Integer, String> tag2Val = new TreeMap();
    ...
    private void outValue(DicomInputStream in) throws IOException {
        ...
        vr.promptValue(val, bigEndian, dcmobj.getSpecificCharacterSet(),
                cbuf, maxValLen, tmpval);
        line.append(tmpval);

        tag2Val.put(tag, tmpval.toString());

        line.append("]");

        if (tag == Tag.SpecificCharacterSet || tag == Tag.TransferSyntaxUID || TagUtils.isPrivateCreatorDataElement(tag)) {
