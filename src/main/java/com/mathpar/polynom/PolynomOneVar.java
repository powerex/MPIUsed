...
public class PolynomOneVar extends Polynom {
     ...
     * @return остаток от деления и домножитель (for n-m+1),
     * остаток от деления и домножитель (минимальный)
     */
    public  static Polynom[] remainderZx(Polynom p, Polynom q, Ring ring) {
        ...
        if (p.powers.length == 0 || p.powers[0] < q.powers[0]) {
            return new Polynom[] {p, p.myOne(ring),p, p.myOne(ring)};}
        Element firstmonQ = q.coeffs[0];
        int t = 0, pw = p.powers[0] - q.powers[0] + 1;
        Polynom chastnoe = new Polynom();
        chastnoe.coeffs = new Element[1];
        boolean not_oneFlag = !firstmonQ.isOne(ring); 
        Element gcd1monomCf=   ring.numberONE;  //; 
        Element multiplier =  ring.numberONE;  //; 
        Element Multiplier =  ring.numberONE;  //; 
        while (p.powers.length != 0 && q.powers[0] <= p.powers[0]) {
            if (not_oneFlag) {gcd1monomCf= firstmonQ.GCD(p.coeffs[0], ring);
                 multiplier = firstmonQ.divideExact(gcd1monomCf, ring);
                 Multiplier = Multiplier.multiply(multiplier, ring); 
                 p = p.multiplyByNumber(multiplier, ring);}           
            chastnoe.coeffs[0] = p.coeffs[0];
            if (p.powers[0] == q.powers[0]) {chastnoe.powers = new int[] {};
            } else {chastnoe.powers = new int[] { p.powers[0] - q.powers[0]}; }
            p = interimRem(p, q, chastnoe, ring);
            t++;
        }  
        Element FQ_pow_wp= (not_oneFlag )?  firstmonQ.pow(pw, ring):ring.numberONE;
        Polynom x=p;
        Element ee2=ring.numberONE;
        if (not_oneFlag ){ ee2=FQ_pow_wp.divideExact(Multiplier, ring);  x = p.multiplyByNumber(ee2, ring);} 
        return new Polynom[] {x, polynomFromNumber(FQ_pow_wp, ring), p, polynomFromNumber(ee2, ring) };
    }
    ...
    /**      Для случая целых коэффициентов и одно переменной. 
    ...
    public Polynom[] ExtendedGCD(Polynom r,Ring ring) {Element one= one(ring);
        ...
        if (term1[1].powers[0] >= term2[1].powers[0]) {
            ...
            if (term2[1].coeffs.length != 0) {
                ...
                result[2] = term[1]; // 'это кривое присвоение виновато, надо наоборот'
                result[1] = term[0];
            }
        }
...
//        q2 = q1.divide(gcdBI, ring);
        ...
        if (p.powers[0] >= q.powers[0]) {
            ...
            beta = v + 2;
            d = braunDiZx(d, v, C1, ring);
            Polynom [] rem=   remainderZx(p, q, ring);
            p=rem[0];
            result[0] = p.divideExact(rem[3],ring); //.multiplyByNumber((q2.pow(beta, ring)).multiply(gcdBI, ring), ring);
        } else {
            C1 = p.coeffs[0];
            v = q.powers[0] - p.powers[0];
            alfa = v + 2;
            ...
            Polynom [] rem=   remainderZx(q, p, ring);
            q=rem[0];
         //   q = remZx(q,p, ring);
            result[0] = q.divideExact(rem[3],ring) ;  
                    //.multiplyByNumber((p2.pow(alfa, ring)).multiply(gcdBI, ring), ring);
        }
        // Последующие деления двух полиномов по алгоритму Евклида с применением
        // упрощения по методу Брауна
        while (p.powers.length > 0 && q.powers.length > 0) {
            if (p.powers[0] > q.powers[0]) {
                ...
                p = braunUprZx(p,d, v, C, ring);
                d = braunDiZx(d, v, C1, ring);
                result[t] = p.divideExact(rem[3],ring) ; 

//.multiplyByNumber(p2.pow(alfa, ring). multiply(q2.pow(beta, ring), ring).multiply(gcdBI, ring), ring);
            } else {
             ...
             //   q = remZx(q,p, ring);
                ...
                q = braunUprZx(q,d, v, C, ring);
                d = braunDiZx(d, v, C1, ring);
                result[t] = q.divideExact(rem[3],ring) ;; //.multiplyByNumber((p2.pow(alfa, ring)).multiply(q2.pow(beta, ring), ring).multiply(gcdBI, ring), ring);
            }t++;
        }
        //вывод ответа
        if (t < dl) {
