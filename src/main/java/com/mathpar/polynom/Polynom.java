...
//                        PascalTriangle.binomialZ(k[i], k[i + 1]).numbElementType(), ring), ring);
     ...
     * @param q делитель
     *
     * @return массив полиномов: на нулевом месте стоит остаток деления this на * q; на первом - домножитель 
     * на втором минимальный остаток, на третьем частное домножителей для первого и для второго
     */
    public Polynom[] remainder(Polynom q, Ring ring) {
 //       System.out.println("p="+this+"   q="+q);
        int vars = powers.length / coeffs.length;
        int v2 = q.powers.length / q.coeffs.length;
        if (vars > v2) {return new Polynom[] { Polynom.polynomZero, q,Polynom.polynomZero, q};
        } 
        if (vars < v2 || powers[vars - 1] < q.powers[v2 - 1]) {
            return new Polynom[] {this, myOne(ring),this, myOne(ring)};
        }
        if (vars == 1) {return PolynomOneVar.remainderZx(this, q, ring);}
        Polynom chastnoe;
        ...
        boolean not_oneFlag = !firstmonQ.isOne(ring);
        Element Multiplier=ring.numberONE;
        // деление полиномов
        while (p.coeffs.length != 0
            ...
            Element gcd1monomCf= firstmonQ.GCD(chastnoe, ring);
    //        System.out.println("Multiplier. gcd1monomCf==="+ Multiplier+"     "+gcd1monomCf);
            Multiplier= Multiplier.multiply(gcd1monomCf, ring);
                 System.out.println("Multiplier. =="+ Multiplier );
            int diff_pow=p.powers[vars - 1] - q.powers[vars - 1];
    //         System.out.println(chastnoe.coeffs.length+"=cfl;    "+p+
      //               "="+p.powers[vars - 1]+"   q="+q.powers[vars - 1]+"   diff_pow="+diff_pow);
            if (diff_pow!=0) {for (int i = 0; i < chastnoe.coeffs.length; i++) {
                                   chastnoe.powers[vars * i + vars - 1] = diff_pow; }
            }  
            // домножение полинома p на коэффициент при старшей степени z полинома q
            if (not_oneFlag) {
                p = p.mulSS(firstmonQ, chastnoe.coeffs.length, p.coeffs.length,
                        0, firstmonQ.coeffs.length, ring);
            } else {p = p.subPolynom(chastnoe.coeffs.length, p.coeffs.length);}
            p = p.subtract(q.mulSS(chastnoe, firstmonQ.coeffs.length,
                    q.coeffs.length, 0, chastnoe.coeffs.length, ring), ring);
            t++;
        }
        // Домножение остатка 
        Polynom x=p, domnozh1=firstmonQ.powRecS(pow, ring), domnozh1_div2=myOne(ring);
        if (not_oneFlag && t != pow) {domnozh1_div2=firstmonQ.powRecS(pow - t, ring);
            x = p.mulSS(domnozh1_div2, ring);
        }
        if(!Multiplier.isOne(ring)){ domnozh1_div2=(Polynom)domnozh1_div2.multiply(Multiplier, ring);
        p=(Multiplier.isItNumber())?p.divideByNumber(Multiplier, ring):  p.divideExact(((Polynom)Multiplier), ring);}
    //    System.out.println("PRS==" +x+"  "+domnozh1+"  "+p+"  "+domnozh1_div2 );
        
        return new Polynom[] {x, domnozh1, p, domnozh1_div2 }; // domnozh2= domnozh1 / domnozh1_div2;
    }
...
//    }
 ...
 данная переменная стала старшей, затем вызываем GCDHPolCoeffs(ring).
    ...
    public Element LCM(Element q, Ring ring) {
        Polynom qq=null;
        if(q.isItNumber(ring)) qq=Polynom.polynomFromNumber(q, ring); 
        else if (q instanceof Polynom) qq=(Polynom)q; else
              { ring.exception.append("return null: LCM hasn't proper arguments."); return null;}
            Element p = GCD(qq, ring);
            return multiply(qq.divide(p, ring), ring);
    }
...
//  add 2015 ++++++++++++++
    ...
    public Polynom[] prs(Polynom q, Ring ring) {
        ...
        Polynom  C1, d, p1, q1, p2, q2, gcdcoeff, p = this, C0=Polynom.polynomZero, hon=Polynom.polynomZero;
        ...
        boolean needsMult= !((p1.isOne(ring))&& (q1.isOne(ring)));
        ...
        if (p.powers[vars - 1] >= q.powers[vars - 1]) {
            ...
            p = p.rem(q, ring);
            result[0] = p;
         //   if(!q2.isOne(ring)) mult= q2.powRecS(v+2, ring).mulSS(gcdcoeff,ring);
           if(needsMult) {mult= q2.powRecS(v+1, ring); hon=p2 ;}
        } else {
            C1 = p.highCoeff();
            v = q.powers[vars - 1] - p.powers[vars - 1];
            d = d.braunDi(v, C1, ring);
            q = q.rem(p, ring);
            result[0] = q;
          //    if(!p2.isOne(ring)) mult= p2.powRecS(v+2, ring).mulSS(gcdcoeff, ring);
            if(needsMult){ mult= p2.powRecS(v+1, ring);  hon=q2 ;}
        }
        ...
        if(!hon.isZero(ring))
           for (int i = 0; i < t; i++) {result[i]=result[i].multiply(hon, ring); }
    ...
    /**
     * Процедура возвращающая последовательность остатков при нахождении НОД
     * двух полиномов this и q по алгоритму Евклида. Отличается тем, что дает коєффициенті - домножители 
     * в наименьших степенях. меньше, чем классический Браун.
     *
     * @param q один из исходных полиномов
     *
     * @return массив полиномов - остатков при нахождении НОД двух полиномов по
     * алгоритму Евклида
     */
    public Polynom[] prsExact(Polynom q, Ring ring) {
        int vars = powers.length / coeffs.length, varsQ=q.powers.length / q.coeffs.length;
        if (vars!=varsQ)return new Polynom[0];    // так как старшая переменная должна быть одинаковая!        
        if (vars == 1) {PolynomOneVar t = new PolynomOneVar(this);return t.prs_Zx(q, ring);} // Для одной переменной отдельно
        Polynom  C1, d, p1, q1, p2, q2, gcdcoeff, p = this, C0=Polynom.polynomZero, hon=Polynom.polynomZero;
        ...
        if(!gcdcoeff.isOne(ring)){p2 = p2.divideExact(gcdcoeff, ring);q2 = q2.divideExact(gcdcoeff, ring);}
        // boolean needsMult= !(p1.isOne(ring))&& (q1.isOne(ring))&&(!gcdcoeff.isOne(ring));
        Polynom mult=p2.multiply(q2, ring);  // 'это паразитный множитель (для случая needsMult=true)
        // Первое деление полиномов без упрощения остатка по методу Брауна, но
        // с нахождением следующего di
        if (p.powers[vars - 1] >= q.powers[vars - 1]) {
            C0 = q.highCoeff();
            v = p.powers[vars - 1] - q.powers[vars - 1];
            d = d.braunDi(v, C0, ring);
            Polynom[] rem=p.remainder(q, ring);
            p = rem[0];
            result[0] = rem[2];
            ...
            v = q.powers[vars - 1] - p.powers[vars - 1];
            d = d.braunDi(v, C1, ring);
            Polynom[] rem=q.remainder(p, ring);
            q = rem[0];
            result[0] = rem[2];
        }
        // Последующие деления двух полиномов по алгоритму Евклида с применением
        ...
        while (p.coeffs.length > 0
            ...
            if (p.powers[vars - 1] > q.powers[vars - 1]) {
                ...
                if (p.coeffs.length == 0) {break;}
                p=p.divideExact((d.powRecS(v, ring).mulSS(C0, ring)),ring) ; 
      //                System.out.println("d.braunDi(v, C1=="+d+"  "+v+"   "+C1);
                d = d.braunDi(v, C1, ring);// d=C1^v/d^{v-1}
                Element max=p.gcd_Domain(rem[3], ring);
                result[t] = (max instanceof Polynom)? p.divideExact((Polynom)max,ring): p.divideByNumber(max, ring);              
    //                 System.out.println("p   d  rest =="+p+"     "+d+"    "+result[t]);
  ...
  //                            System.out.println("d.braunDi(v, C1=="+d+"  "+v+"   "+C1);
                d = d.braunDi(v, C1, ring);// d=C1^v/d^{v-1}
                Element max=q.gcd_Domain(rem[3], ring);
                result[t] = (max instanceof Polynom)? q.divideExact((Polynom)max,ring): q.divideByNumber(max, ring);
         //        System.out.println("q  mult  rem[3]   d  rest =="+q+"     "+mult+"     "+rem[3]+"    "+d+"    "+result[t]); 
            } 
             t++; 
        }  // end of while   
        mult=mult.multiply(gcdcoeff, ring);
        if(!mult.isOne(ring))
           for (int i = 0; i < t; i++) {result[i]=result[i].multiply(mult, ring); }
        if (t < dl) {Polynom[] otv = new Polynom[t];
            System.arraycopy(result, 0, otv, 0, t);
            return otv;
        }
    ...
    public VectorS extendedGCD(Polynom r, Ring ring) {
        ...
        if (vars == 1) {Polynom[] pol;
            if(ring.isFeild()) {Element[] euclDom=extendedGCDinEDx(q, ring);          
            Polynom pp=(Polynom)euclDom[0];
            Element first=pp.coeffs[0];
            if (!first.isOne(ring)){for (int i = 0; i < euclDom.length; i++) 
                    euclDom[i]=((Polynom)euclDom[i]).divideByNumber(first, ring);
                }
                return new VectorS(euclDom);}
            PolynomOneVar t = new PolynomOneVar(p);
    ...
    public int compareTo(Polynom p, Ring ring) {
        ...
        if ((length_p == 0) && (length_this == 0)) { return 0;  }
        if (length_p == 0) { return signum();  }
        if (length_this == 0) { return -p.signum();  }
      //  int v = p.coeffs[0].numbElementType();
      //  int m = coeffs[0].numbElementType();
            if (powers.length / coeffs.length > p.powers.length
                    / p.coeffs.length) { return 1;  }
            if (powers.length / coeffs.length < p.powers.length
                    / p.coeffs.length) {  return -1; }
            int leng = Math.min(coeffs.length, p.coeffs.length), 
                    vars = p.powers.length / p.coeffs.length;
            for (int i = 0; i < leng; i++) {
                for (int j = vars - 1; j >= 0; j--) {
                    if (powers[i * vars + j] > p.powers[i * vars + j]) {  return 1; }
                    if (powers[i * vars + j] < p.powers[i * vars + j]) { return -1; }
                }         
                int v = p.coeffs[0].numbElementType();
                int m = coeffs[0].numbElementType(); 
                if (v == m) {
                    ...
                    Element diff = coeffs[i].subtract(p.coeffs[i], ring);
                    if (diff.isNegative()) {  return -1;}
                    if (!diff.isZero(ring)) {return 1;  }
                }
            }
            if (coeffs.length > p.coeffs.length) { return 1;
            } else {
                if (coeffs.length < p.coeffs.length) { return -1; } else {return 0;}
            }
       // }
    }
...
//     */
    ...
    public Polynom integrate(int num, Ring ring) {
        ...
        if (Kol_perem <= num) {
            ...
            for (int i = 0; i < coeffs.length; i++) {
                Powers[n]++;
                Coeffs[i] = Coeffs[i].
                        divide(ring.numberONE.valOf(Powers[n], ring), ring);
                n += Kol_perem;
            }
        }
        return new Polynom(Powers, Coeffs);
 ...
 public FactorPol factorOfPol_inQ(boolean squareFree, Ring ring) {
        if(degreeTotal()<2){Element gcdC=this.GCDNumPolCoeffs(ring);
            return (gcdC.isOne(ring))? 
                    (this.isNegative())? 
                    new FactorPol(new int[]{1,1}, new Polynom[]{Polynom.polynomFromNot0Number(ring.numberMINUS_ONE),(Polynom)this.negate(ring)}):
                    new FactorPol(this):
                    (this.isNegative())? 
                    new FactorPol(new int[]{1,1}, new Polynom[]{Polynom.polynomFromNot0Number(gcdC.negate(ring)),((Polynom)this.negate(ring)).divideByNumber(gcdC, ring)}):
...
//    
    ...
    public FactorPol toCoeffsHighestVar(int numbV) {
        ...
        int[] powarray = new int[powers.length];
        int i = 0;
        int start = 0;
        for (; i < shift; i++) {
            int k = powers[d];
...
//
            ...
            while ((d < powers.length) && (k == powers[d])) {
                System.arraycopy(powers, start, powarray, size, nVars);
                size += nVars;
                start += vars;
                d += vars;
                coef.add(coeffs[j]);
            ...
            int[] p1 = new int[size];
            System.arraycopy(powarray, 0, p1, 0, size);
   ...
   /**
    ...
    public Polynom[] toMonomials(Ring ring) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            m[i] = new Polynom(p, new Element[] {coeffs[i]}).normalNumbVar(ring);
        }
        return m;
    }
...
//    }
    ...
    /** For Euclidean Domain with one var x. 
   ...
   /**
    ...
    public Element arg(Ring ring) {
      ...
      Polynom Re=Re(ring); Polynom Im=Im(ring);
        ...
        /**
     * Редукция массива полиномов по одному
     * @param v
     * @param p
     * @param ring
     * @return
     */
    public VectorS arrayReductionByOne(VectorS v, Polynom p, Ring ring){
        ArrayList<Polynom> basePol = new ArrayList<>();
        for (int i = 0; i < v.V.length; i++)
            basePol.add((Polynom) v.V[i]);
        
        ArrayList<Polynom> pol = new ArrayList<>();
        pol.add(p);
        Polynom red;
        for (int i = 0; i < basePol.size(); i++) {
            red = Buchberger.rem(ring, basePol.get(i), pol);
            if (!red.isZero(ring)) {
                for (int j = 0; j < red.coeffs.length; j++) {
                    Element numCoeff = red.coeffs[j];
                    if (numCoeff instanceof Fraction) {
                        Fraction fr = (Fraction) numCoeff;
                        if (fr.num != fr.denom) {
                            red = red.multiplyByNumber(numCoeff.inverse(ring), ring);
                        }
                    }
                }
                Element c_gcd = red.GCDNumPolCoeffs(ring);
                if (!c_gcd.isOne(ring)) {
                    red = red.divideByNumber(c_gcd, ring);
                }
                if (red.isNegative()) {
                    red = (Polynom) red.negate(ring);
                }
            }
            System.out.println("Редукция: [" + basePol.get(i) + ", " + pol + "]  =  " + red);
            if(red.isZero(ring)){
                basePol.remove(i);
                i--;
                continue;
        ...
        VectorS out = new VectorS(basePol.size());
        for (int i = 0; i < out.V.length; i++)
            out.V[i] = basePol.get(i);
        return out;
    }
    
    
    /**
     * Возвращает gcd и коэффициенты Безу как на сайте (3 элемента)
     *
     * @param q второй полином
     * @param ring кольцо
     *
     * @return [gcd, a, b]
     */
    public VectorS extendedGCD2(Element q, Ring ring) {
        VectorS egcd = this.extendedGCD(q, ring);
        if (egcd.V.length == 3) {
            return egcd;
        }
        ...
        res[1] = egcd.V[1];
        res[2] = egcd.V[2];
        Element den = egcd.V[5].multiply(egcd.V[3], ring);
        if (!den.isOne(ring)) {
            res[1] = new Fraction(egcd.V[1], den).cancel(ring);
        }
        Element den2 = egcd.V[5].multiply(egcd.V[4], ring);
        if (!den2.isOne(ring)) {
            res[2] = new Fraction(egcd.V[2], den2).cancel(ring);
        }
        return new VectorS(res);
    }
    /**
     * extendedGCD для массива полиномов
     *
     * @param base массив полиномов
     * @param ring кольцо
     *
     * @return [gcd, a, b, c, d ... ] - домножителиs
     */
    public VectorS arrayExtendedGCD2(Element[] base, Ring ring) {
        Element[] coeff = new Element[base.length + 1], egcd;
        if(base.length == 1){
            coeff[0] = base[0];
            coeff[1] = ring.numberONE;
            System.out.println("arrExtGCD("+Array.toString(base)+") = "+Array.toString(coeff));
            return new VectorS(coeff);
        }
        for (int i = 0; i < coeff.length; i++) {
            coeff[i] = ring.numberZERO;
        }

        if (base[0].isItNumber(ring)) {
            coeff[0] = ring.numberONE;
            coeff[1] = ring.numberONE;
        ...
        Polynom gcd = (Polynom) base[0];
        int var_num = gcd.powers.length / gcd.coeffs.length;
        //System.out.println(Array.toString(base));
        for (int i = 1; i < base.length; i++) {
            if (base[i].isItNumber(ring)) {
                coeff[0] = ring.numberONE;
                coeff[i + 1] = ring.numberONE;
                for (int j = 1; j < coeff.length; j++) {
                    if (i + 1 != j) {
                        coeff[j] = ring.numberZERO;
                    }
                }
                System.out.println("arrExtGCD("+Array.toString(base)+") = "+Array.toString(coeff));
                return new VectorS(coeff);
            }
            egcd = gcd.extendedGCD2(base[i], ring).V;
            //System.out.println("extendedGCD(" + gcd + ", " + base[i] + ") = " + Array.toString(egcd));
            gcd = (Polynom) egcd[0];
            if (i == 1) {
                coeff[1] = egcd[1];
                coeff[2] = egcd[2];
                ...
                for (int j = 1; j < i + 1; j++) {
                    coeff[j] = coeff[j].multiply(egcd[1], ring);
                }
                coeff[i + 1] = egcd[2];
            }
            //System.out.println(Array.toString(coeff));
            if (gcd.powers.length / gcd.coeffs.length != var_num) {
                break;
            }
        }
        coeff[0] = gcd;
    ...
    public Polynom subPrs(Polynom q, Ring ring){
        if(this.powers.length/this.coeffs.length != q.powers.length/q.coeffs.length)
            return null;
        int high = this.compareTo(q, ring);
        Element prs;
        Polynom p1, p2;
        if(high == 1){
            p1 = q;//младший
        ...
        int p1deg = p1.degree(ring.varPolynom.length-1);
        int p2deg = p2.degree(ring.varPolynom.length-1);
        if(p1deg < 0) p1deg = 0;
        if(p2deg < 0) p2deg = 0;
        int k = p2.degree(1)-p1.degree(1);
        Element c = p2.coefOfHightVar(1, ring).divide(p1.coefOfHightVar(1, ring), ring);
        //Пусть p1 младший и р2 - старший. Вычислим:
        //р3= p2 - p1* y^k;
        //где k -- разность степеней по у
        prs = p2.subtract(p1.multiply(ring.varPolynom[1].pow(k, ring).multiply(c, ring), ring), ring);
        if(prs.isZero(ring)) return null;
        return (Polynom) prs;
    }
    public VectorS groebner2var(Polynom[] input, Ring ring){
        //сортируем вход в порядке возрастания
        //определяем максимальную степень старшей переменной в полиномах
        //группируем по степеням
        input = Array.sortUpMod(input, ring);
        System.out.println("После сортировки по возрастанию: "+Array.toString(input));
        int max_pow = input[input.length-1].powers[1];//наибольшая степень
        int first = 0;//наименьшая степень
        System.out.println("Наибольшая степень = "+max_pow);
        VectorS[] group = new VectorS[max_pow+1];
        for (int i = 0; i < group.length; i++)
            group[i] = new VectorS(0);
        for (int i = 0; i < input.length; i++) {
            if(input[i].maxPowOfVars__(ring).length < ring.varPolynom.length){ 
                group[0] = group[0].addElement(input[i]);
                first = 0;
            }
            else{
                int pow = input[i].maxPowOfVars__(ring)[ring.varPolynom.length-1];
                group[pow] = group[pow].addElement(input[i]);
                first = pow;
            }
        }
        for (int i = 0; i < group.length; i++)
            System.out.println("Группа "+i+": "+group[i]);
        
        //
        //
        //
        int minModifyDeg;   //наименьшая из групп, куда попали новые полиномы
        for (int currentGroup = 0; currentGroup < group.length; currentGroup++) {
            if(group[currentGroup].length() == 0) continue;//пропускаем группу если пустая
            int nextGroup = currentGroup+1;//следующая группа
            if(nextGroup < group.length)
                while(group[nextGroup].length() == 0){
                    nextGroup++;
                    if(nextGroup >= group.length) break;
                }
            minModifyDeg = group.length;
            VectorS gcd; 
            
            if(currentGroup == first && group[currentGroup].length() > 1){//действие с первой группой
                System.out.println("Группа "+currentGroup+".");
                //если группа 0 степени, то egcd делаем с полиномами
                if(currentGroup == 0){
                    gcd = arrayExtendedGCD2(group[currentGroup].V, ring);
                    if(gcd.V[0].isOne(ring)) return new VectorS(new Fname("Должен вернуться массив input"));
                }
                //иначе делаем egcd с коэффициентами полиномов
                else{
                    gcd = new VectorS(group[currentGroup].length());
                    for (int i = 0; i < gcd.length(); i++)
                        gcd.V[i] = ((Polynom) group[currentGroup].V[i]).coefOfHightVar(1, ring);
                    gcd = arrayExtendedGCD2(gcd.V, ring);
                }
                //собираем новый полином
                Element new_el = ring.numberZERO;
                for (int k = 0; k < group[currentGroup].length(); k++){
                    if(gcd.V[k+1].isZero(ring)) continue;
                    if(gcd.V[k+1].isOne(ring)){
                        new_el = new_el.add(group[currentGroup].V[k], ring);
                        continue;
                    }
                    new_el = new_el.add(group[currentGroup].V[k].multiply(gcd.V[k+1], ring), ring);
                }
                System.out.println("Новый полином egcd: "+new_el);
                //выполняем prs для каждого полинома группы с новым
                //результат записываем в промежуточный буфер
                VectorS buffer = new VectorS(0);
                for (int i = 0; i < group[currentGroup].length(); i++){
                    if(new_el.subtract(group[currentGroup].V[i], ring).isZero(ring)) continue;
                    Polynom[] prs = null;
                    if(((Polynom)new_el).compareTo((Polynom) group[currentGroup].V[i], ring) == 1)
                        prs = ((Polynom)new_el).prsExact((Polynom) group[currentGroup].V[i], ring);
                    else
                        prs = ((Polynom)group[currentGroup].V[i]).prsExact((Polynom) new_el, ring);
                    if(prs.length == 0) continue;
                    //пробегаем по всем prs
                    for (int j = 0; j < prs.length; j++) {
                        if(prs[j].isZero(ring)) continue;
                        buffer = buffer.addElement(prs[j]);
                    }
                }
                //раскидываем элементы из буфера по группам
                for(int i = 0; i < buffer.length(); i++){
                    int degNew = ((Polynom)buffer.V[i]).degree(ring.varPolynom.length-1);
                    if(degNew < 0) degNew = 0;
                    System.out.println("Новый полином prs: "+buffer.V[i]);
                    if(group[degNew].exist(buffer.V[i], ring) == false){
                        group[degNew] = group[degNew].addElement(buffer.V[i]);
                        if(degNew < minModifyDeg) minModifyDeg = degNew;
                    }
                }
                group[currentGroup].V = new Element[]{new_el};
                if(first > minModifyDeg) first = minModifyDeg;
                if(minModifyDeg <= currentGroup){
                    currentGroup = minModifyDeg - 1;
                    continue;
                }
            }            
            if(nextGroup >= group.length) break;
            //теперь берем по две группы, начиная с 1 непустой
            //соединяем их в одну группу
            System.out.println("Группа "+currentGroup+","+nextGroup+".");
            VectorS commonGroup = group[currentGroup].merge(group[nextGroup]);
            //теперь надо выделить коэффициенты в группах
            //если первая группа степени 0, то они и есть коэффициенты, иначе
            //выделяем коэффициенты в первой группе
            Element[] currentCoeff = new Element[group[currentGroup].length()];
            for (int k = 0; k < currentCoeff.length; k++){
                if(currentGroup == 0) currentCoeff[k] = group[currentGroup].V[k];
                else currentCoeff[k] = ((Polynom) group[currentGroup].V[k]).coefOfHightVar(1, ring);
            }
            //ну а во второй группе степень по-любому больше 0
            Element[] nextCoeff = new Element[group[nextGroup].length()];
            for (int k = 0; k < nextCoeff.length; k++)
                nextCoeff[k] = ((Polynom) group[nextGroup].V[k]).coefOfHightVar(1, ring);
            //объединяем коэффициенты в одну группу
            VectorS commonCoeffs = new VectorS(currentCoeff).merge(new VectorS(nextCoeff));
            //находим gcd
            gcd = arrayExtendedGCD2(commonCoeffs.V, ring);
            Element new_el = ring.numberZERO;
            //пока находимся среди коэффициентов первой группы, домножаем их на 
            //старшую переменную в степени (nextGroup-currentGroup)
            Element slag;
            int one = 0, zero = 0, index = 0;
            for (int i = 1; i < gcd.length(); i++) {
                if(!gcd.V[i].isZero(ring) && !gcd.V[i].isOne(ring)) break;
                if(gcd.V[i].isOne(ring)){one++; index = i;}
                if(gcd.V[i].isZero(ring)) zero++;
            }
            if(one == 1 && zero == gcd.length()-2)
                new_el = commonGroup.V[index-1];
            else
                for (int i = 0; i < commonGroup.length(); i++){
                    if(i < currentCoeff.length) slag = commonGroup.V[i].multiply(gcd.V[i+1], ring)
                            .multiply(ring.varPolynom[1], ring);
                    else slag = commonGroup.V[i].multiply(gcd.V[i+1], ring);
                    new_el = new_el.add(slag, ring);
                }
            System.out.println("Новый полином egcd: "+new_el);
            //если в egcd выпал младший полином
            if(((Polynom)new_el).degree(1) < nextGroup){
                System.out.println("Младший убил старшего!");
                int groupSize = group[nextGroup].length();
                VectorS buffer = new VectorS(0);
                for (int i = 0; i < groupSize; i++) {
                    Polynom prs = ((Polynom)group[nextGroup].V[i]).subPrs((Polynom) new_el, ring);
                    if(prs != null)
                        buffer = buffer.addElement(prs);
                }
                for(int i = 0; i < buffer.length(); i++){
                    int degNew = ((Polynom)buffer.V[i]).degree(ring.varPolynom.length-1);
                    if(degNew < 0) degNew = 0;
                ...
                group[nextGroup].V = new Element[0];
                if(first > minModifyDeg) first = minModifyDeg;
                if(minModifyDeg <= currentGroup){
                    currentGroup = minModifyDeg - 1;
                    continue;
            ...
            //выполняем prsExact для каждого полинома группы с новым
            else{
                VectorS buffer = new VectorS(0);
                for (int i = 0; i < commonGroup.length(); i++){
                    if(new_el.subtract(commonGroup.V[i], ring).isZero(ring)) continue;
                    Polynom[] prs = ((Polynom)new_el).prsExact((Polynom) commonGroup.V[i], ring);
                    if(prs.length == 0) continue;
                    //пробегаем по всем prs
                    for (int j = 0; j < prs.length; j++) {
                        if(prs[j].isZero(ring)) continue;
            ...
            group[nextGroup].V = new Element[]{new_el};
            if(first > minModifyDeg) first = minModifyDeg;
            if(minModifyDeg <= currentGroup)
                currentGroup = minModifyDeg - 1;
        }
        //теперь у нас в каждой непустой группе по 1 полиному

        //объединение групп и вывод ответа
        for (int i = 2; i < group.length; i++)
            group[1] = group[1].merge(group[i]);
        if(group[0].length() > 0)
            group[1] = arrayReductionByOne(group[1], (Polynom)group[0].V[0], ring);
        group[0] = group[0].merge(group[1]);
        System.out.println("Решение: \t\t"+group[0].toString());
        VectorS right = com.mathpar.polynom.gbasis.util.CommandsRunner.runGbasis(ring, input);
        System.out.println("Правильное решение: \t"+right.toString());
        return group[0];
    }
...
Polynom f10 = new Polynom(" x^2 ", ring);
Polynom f11 = new Polynom(" (1/2)x ", ring);
System.out.println("comp="+f10.compareTo(f11, ring));
...
////        powers[45] = 1;
//     
//        Polynom p11 = new Polynom(" yx^2-x^3", ring);
//   //  FactorPol ffpp=   p.factor( ring);
//        Polynom p = new Polynom("y^3+x", ring); // Polynom p1=  new Polynom("3y^2x+3z^2x", ring); 
//    //    p=p.multiply(p1, ring).multiply(p1, ring);
//        System.out.println(p.toString(ring));        
//               Polynom[] fp= p.remainder(p11,   ring);
//        for (int i = 0; i < fp.length; i++) {
// 
//               System.out.println(
//              "   fp ="+fp[i].toString(ring));}
//                 System.out.println();
// 
   }
}