...
public class FactorPol extends Element {
  //  private static final long serialVersionUID = -4404752564620149248L;
    ...
    public FactorPol multiply(FactorPol b, Ring ring) {
        b.normalFormInField(ring);
        normalFormInField(ring);
        int[][] pow = new int[1][0];
        ...
        int start = 0;
        if ((l > 1) && res[0].isItNumber() && res[1].isItNumber()) {// было так: if ((l > 1) && (res[1].signum() < 0)) {
            Element number = res[0].pow(pow[0][0], ring).multiply(res[1].pow(pow[0][1], ring), ring);
            one = (number instanceof Polynom)? (Polynom) number:
                    new Polynom(new int[]{}, new Element[]{number});
            if (one.isOne(ring)) {
                start = 2;
            } else {
                start = 1;
