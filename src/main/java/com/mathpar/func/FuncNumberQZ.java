...
public class FuncNumberQZ {
    ...
    public FuncNumberQZ(Ring r) {
        ...
        // sqrt(3) / 3
        sqrt_3_div_3 =  new Fraction(sqrt_3, z3);   //  new F(F.DIVIDE, new Element[] {sqrt_3, z3});
        // - sqrt(3) / 3
        neg_sqrt_3_div_3 =   new Fraction(neg_sqrt_3, z3);         new F(F.DIVIDE, new Element[] {
    ...
    public Element arc_trigFunc_Grad(Element znach, int funcNumb, Ring ring) {
        // избавимся оь иррациональности в знаменателе
        if (znach instanceof Fraction){Fraction fr=(Fraction)znach; 
            if (fr.denom instanceof F){znach=fr.disTransInDenom(ring);}  }
        switch (funcNumb) {
            case F.SIN: {
            ...
            case F.TG: {
                ...
                if (znach instanceof Fraction) {Fraction fr= (Fraction)znach;
                 if(!((fr.num instanceof Polynom)||(fr.denom instanceof Polynom)))
                    for (int i = 0; i < TablZnachForTG.length; i++) {
                        if (TablZnachForTG[i] instanceof Fraction ) {
                            if ((((Fraction) znach).subtract(TablZnachForTG[i], ring)).isZero(ring)){
                                return TablGradFor_SIN[i];
                            }
                        }
                    }
