 ...
 */
    ...
    public VectorS solveAlgMatrixs(VectorS vec, int[] indexs, boolean flag) {
...
//==============================================================================
 ...
 // #############################################################################                   
                ...
                case F.REMAINDER: 
                   //     return  ((Polynom)CalcArg[0]).remainder(((Polynom)CalcArg[1]), RING)[2];
                case F.QUOTIENT:  
...
//                    return new VectorS(pp);
                ...
                //    Polynom[] pp= pvv.prs((Polynom)CalcArg[1], RING);
                    return new VectorS(pvv.prsExact((Polynom)CalcArg[1], RING));
  ...
  //  (str.1727)
                case F.INT:
                    Element resInt = new Integrate().integrate(CalcArg[0], CalcArg[1], RING);
                    return   resInt;
               ...
               case F.FACTORINC:
                ...
                case F.MODCENTERED:
                    return CalcArg[0].Mod(CalcArg[1],RING);
                case F.MODINVERSE:
                    return CalcArg[0].modInverse(CalcArg[1]);
                case F.ROUND:
...
//-------------------------------------------------------------------------------------------
              ...
              case F.RANK: {return ((MatrixD) CalcArg[0]).rank(newRing);
                ...
                case F.B_LE:
                case F.B_LESS:    
                   CalcArg=helpToBooleanFunctions(CalcArg); F bF=new F(func.name, CalcArg);
                   return(CalcArg[0].isItNumber() && CalcArg[1].isItNumber()) ? bF.booleanFunction(RING) : bF;
                case F.B_AND:     
                case F.B_OR:
                case F.B_IMPLIES:
                case F.B_EQUIV:
                    CalcArg=helpToBooleanFunctions(CalcArg); 
                   if((CalcArg[0] instanceof MatrixD)||(CalcArg[0] instanceof MatrixS)||(CalcArg[0] instanceof VectorS)||(CalcArg[0] instanceof VectorS)) 
                       return (F.B_OR==func.name)? CalcArg[0].B_OR(CalcArg[1], RING): (F.B_AND==func.name)? CalcArg[0].B_AND(CalcArg[1], RING): Element.NAN ;
                ...
                case F.SOLVEDE:{ 
                    ...
                    return new SolveDE().solve(CalcArg[0], CalcArg[1]);
                } 
//              case F.SOLVEHDE:{
        ...
        for (int i = 0; i < rows.length; i++) {
            switch (rows[i].numbElementType()) {
                case Ring.F:
                    ...
                    newVec[i] = 
                            (((F)el).name==F.VECTORS)?
                            SVTFACPtoMatrixsAndVectors(((F)el).X): 
                            El2Pol((F) el);
                    t++;
                    break;
                case Ring.Fname:
...
//                    }
        ...
        switch (nameF) {
            ...
            case F.SUBTRACT:
                return(substituteArg[0].numbElementType()<=Ring.Polynom & substituteArg[1].numbElementType()<=Ring.Polynom)? 
                        substituteArg[0].subtract(substituteArg[1], RING):
   ...
   //    System.out.println("CanonicForms.ListOfChanges="+ Array.toString( List_of_Change.toArray()));            
    ...
    public Element workToDefault(Element El, int Pow) {Polynom temp_=null;
        ...
        if (pos >= 0) { //если нашли ,то берем с тем же индексом из List_of_Polynom            
            ...
            return  (Pow==1)?    
                    (Polynom)List_of_Polynom.get(pos):
                    ((Polynom)List_of_Polynom.get(pos).clone()).pow(Pow, RING);
...
//========================================================================================
    ...
    /**
     * Процедура конвертирующая FactorPol в F с учетом List_of_Cange и
     * List_of_Polynom
     *
     * @param p - входной полином
     *
     * @return
     */
    public Element Unconvert_FactorPol_to_Element(Element el) {
        FactorPol fp=(FactorPol)el;
        int m=fp.multin.length;
        Element PN = Polynom.polynom_one(RING.numberONE);
        Element PD = Polynom.polynom_one(RING.numberONE);
        for (int i = 0; i < m; i++) {
            if (fp.powers[i] >= 0) {
                PN = PN.multiply(Unconvert_Polynom_to_Element(fp.multin[i]).pow(fp.powers[i], RING), RING);
            } else {
                PD = PD.multiply(Unconvert_Polynom_to_Element(fp.multin[i]).pow(-fp.powers[i], RING), RING);
            }
        }
        return (PD.isOne(RING))? PN: // (PD.isItNumber())? PN.divideByNumber(PD.coeffs[0], ring): 
                new Fraction(PN, PD);
    }
...
//                return vecF2P((VectorS) el);
            case Ring.FactorPol:
                return Unconvert_FactorPol_to_Element(el);
            case Ring.Polynom:
                return Unconvert_Polynom_to_Element(el);
        ...
        switch (el.numbElementType()) {
            case Ring.F: {
                ...
                switch ((Fel).name) {
                    ...
                    case F.SUBTRACT: {
                        if (((Fel).X[0]==(Fel).X[1])&&
                          ( ((Fel).X[1].numbElementType()!=Ring.MatrixD) 
                                &&  ((Fel).X[1].numbElementType()!=Ring.MatrixS) &&
                                ((Fel).X[1].numbElementType()!=Ring.VectorS))) return RING.numberZERO;
                    ...
                    case F.DIVIDE: {if ((Fel).X[0]==(Fel).X[1])return RING.numberONE;
                    ...
                    case F.POW:
                        nArg = new Element[2];
                        nArg[0] = expand_next_level(Fel.X[0]);
                        if(Fel.X[1].isItNumber() && Fel.X[1].Im(RING).isZero(RING) &&
                                (Fel.X[1].toNumber(Ring.R64, RING).floor(RING).subtract(Fel.X[1], RING).isZero(RING)) ) {
                            return funcExpand(CleanElement(new F(F.intPOW, nArg[0], Fel.X[1].toNumber(Ring.Z, RING))));
                        }
                        nArg[1] = expand_next_level(Fel.X[1]);
                        return workToDefault(CleanElement(new F(((F) el).name, nArg)), 1);
                    case F.SYSTLDE:
                    case F.SYSTEM:
                    case F.EQUATION:
                    case F.SYSTEMOR:
                    ...
                    default:                      
                        if( F.isInfixName(Fel.name)){
                           for (int i = 0; i < Fel.X.length; i++) Fel.X[i]=funcExpand((Fel).X[i]);                  
                           return  Fel ;}
...
//                          //  return workToDefault(temp, 1);
                        ...
                        Element temp = powFSP(((F) el).X,false,true);
                        temp = CleanElement(temp);
                        
                        if (((F)temp).name == F.ID) {
                            temp = ((F)temp).X[0];
                        }
                        
                        if (temp instanceof F == false) {
                            return workSWG(temp);
                        }
                        
                        if (((F)temp).name == F.intPOW) {
                            if (((F)temp).X[1].isNegative()) {
                                temp = workSWG(((F) temp).X[0]).pow(((F) temp).X[1].multiply(RING.numberMINUS_ONE, RING), RING);
                                return RING.numberONE.divide(temp, RING);
                            } else {
                                return workSWG(((F) temp).X[0]).pow(((F) temp).X[1], RING);
                            }
                        } else if (((F)temp).name == F.POW) {
                            if ( ( ((F)temp).X[1].numbElementType() < Ring.Polynom) && (((F)temp).X[1].isNegative()) ) {
                                ((F)temp).X[1] = ((F)temp).X[1].multiply(RING.numberMINUS_ONE, RING);
                                return new Fraction(RING.numberONE, workToDefault(temp, 1));
                            } else {
                                return workToDefault(temp, 1);
                            }
                        } else {
...
//                        
//                            Element temp=new F(F.POW, new Element[]{ElementToF(
//                                    workSWG(((F) el).X[0])),
//                                ElementToF(workSWG(((F) el).X[1]))}
//                            );
//                            temp=F.cleanOfRepeating(temp, RING);
//                            F ff=powFSP(((F) temp).X,false,true);
//                            
