...
public class Fname extends Element {
     ...
     *
     * [0] - нижние индексы (характерно для обозначения элементов матриц
     * векторов,тензоров а так же переменных с нижними индексами)
     *
     * [1] - верхние индексы (характерно для тензоров)
     *
     * [2] - индексы переменных из кольца (характерно для обозначения функций)
     */
    ...
    public Fname(String n, Element[][] ind) {
        this.name = n;
        this.indices=ind;
    }
    public Fname(String n, int[][] ind) {
        this.name = n;
        for (int i = 0; i < ind.length; i++) {
          if (ind[i]!=null)  { Element[] row= new Element[ind[i].length];
              for (int j = 0; j < ind[i].length; j++) row[j]= Element.POSCONST[ind[i][j]];
              this.indices[i]=row; 
          }       
        }
    }  
    /** Создание специальных объектов этого класса
     * 
     * Заполняем индексы:
     * [Indrow=1] - нижние индексы   для обозначения элементов матриц
     * векторов,тензоров а так же переменных с нижними индексами
     * [Indrow=2] - верхние индексы для тензоров
     * [Indrow=3] - индексы переменных из кольца для обозначения функций от этих переменных
     */
    public void setIndeces( int[] ind, int Indrow) {
         Element[] row= new Element[ind.length];
          for (int j = 0; j < ind.length; j++) row[j]= Element.POSCONST[ind[j]];
          this.indices[Indrow]=row; 
      }       

    public void setFuncX(){
      this.indices[2]= new Element[]{NumberZ.ZERO};
      }       
    public void setFuncY(){
            this.indices[2]= new Element[]{NumberZ.ONE}; 
    }       

    public void setFuncXY(){
         this.indices[2]= new Element[]{NumberZ.ZERO, NumberZ.ONE};
          }
    ...
    */
    ...
    public Element sin(Ring ring) {
        return (name.equals("\\pi")) ? NumberZ.ZERO : new F(F.SIN, this);
    }

    @Override
    public Element cos(Ring ring) {
        return (name.equals("\\pi")) ? NumberZ.MINUS_ONE : new F(F.COS, this);
    }

    @Override
    public Element tan(Ring ring) {
        return (name.equals("\\pi")) ? NumberZ.ZERO : new F(F.TG, this);
    }

    @Override
    public Element ctg(Ring ring) {
        return (name.equals("\\pi")) ? Element.NAN : new F(F.CTG, this);
    }
...
/**
 * Derivative by the variable ring.varPolynom[num].
 * varsIndices = Index[3] is the array with all variables of this function.
 * If it is [2,3] than the function of z and t. So the result is
 * D(this, [num]) if num=2 or num=3. Other cases for X=null result is 0 and X!=null result X[0].D(num, ring).
 * 
 * @param num - the variable of differentiation as it is in Ring ring
 * @param ring - Ring
