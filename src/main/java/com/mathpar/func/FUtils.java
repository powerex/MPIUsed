...
//            }
            ...
            if(indexBackSlash!=-1){ int ss=indexBackSlash+2; char SH=res.charAt(ss);
                 while((ss<index)&&(!(SH==' '||SH=='_'||SH=='('||SH=='['||SH=='{'))){ss++; if(ss<index)SH=res.charAt(ss);}
                 String str1=res.substring(indexBackSlash+1, ss);
                 Integer na= F.strMap.get(str1); 
                 if( (na==null)||(F.isInfixName(na))) indexBackSlash=-1;
            }
                 ...
                 (!((index>1)&&(res.charAt(index-1)=='{')&&(res.charAt(index-2)=='^')))) // <- показатель производной сохраняем в скобках
            {
                ...
                if ((((indexO2 != -1) && (indexO2 < index)) || ((index < posPareFB) && (index > indexFB))
                     ...
                     (!((index>1)&&(res.charAt(index-1)=='{')&&(res.charAt(index-2)=='^'))) && posPare != -1) // <- показатель производной сохраняем в скобках
                {
                    res.deleteCharAt(posPare);
                    res.deleteCharAt(index);
                    pp = index;
 ...
 */
    ...
    public static String toStringD_new(String w, String ind,  Ring ring) {
       ...
       public static String toStringD(String w, String ind,  Ring ring) {
        ...
        else if(x.length==2){  res.append("\\partial_{").append(x[1]).append("}(").append(x[0]).append(")");}
...
//    }
    ...
    public static String[] cutByCommas(String W, Ring ring) {
        ...
        while(W.indexOf("\\,",p-1)==p-1) p  = W.indexOf(',',p+1); if (p==-1)return new String[]{W};
        int len = W.length(); 
        int[] zap = new int[len];
        ...
        while(p>0){
            ...
            p=W.indexOf(',',BFQ);  
            while((p>-1)&&(W.indexOf("\\,",p-1)==p-1)) p  = W.indexOf(',',p+1);
            pB= W.indexOf('(',BFQ); 
            pF= W.indexOf('{',BFQ);
            pQ= W.indexOf('[',BFQ); }  
        zap[i++] = len;
