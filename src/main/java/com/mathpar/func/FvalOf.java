...
public class FvalOf {
    ...
    public Element valOf_(F f, Element[] point) {
        ...
        switch (f.name) {
            ...
            case F.B_OR:
               if((CalcArg[0] instanceof NumberZ64) && (CalcArg[1] instanceof NumberZ64))
                return  ((NumberZ64) CalcArg[0]).B_OR(((NumberZ64) CalcArg[1]));
                else return (CalcArg[0].isOne(ring))||(CalcArg[1].isOne(ring))? Element.TRUE:
                        (CalcArg[0].isZero(ring))? CalcArg[1]: (CalcArg[1].isZero(ring))? CalcArg[0]:
                        new F(f.name, CalcArg);
            case F.B_AND: 
                if((CalcArg[0] instanceof NumberZ64) && (CalcArg[1] instanceof NumberZ64))
                return  ((NumberZ64) CalcArg[0]).B_AND(((NumberZ64) CalcArg[1]));
                else return (CalcArg[0].isZero(ring))||(CalcArg[1].isZero(ring))? Element.FALSE:
                        (CalcArg[0].isOne(ring))? CalcArg[1]: (CalcArg[1].isOne(ring))? CalcArg[0]:
                        new F(f.name,CalcArg);
            case F.ID:
