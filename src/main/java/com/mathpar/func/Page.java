...
public class Page {
    ...
    public static final Pattern VECT_MATRIX =
            Pattern.compile(" *\\(*[a-zA-Z][a-zA-Z]*[0-9a-zA-Z]*_\\{(.*,)*.*}\\)* *");
    /**
    ...
    //////////////////////////////////////////////////////
    ////Налаштування для 3D/////////
    public double xMin3D = MarchingCubes.DEFAULT_X_MIN;
    public double xMax3D = MarchingCubes.DEFAULT_X_MAX;
    public double yMin3D = MarchingCubes.DEFAULT_Y_MIN;
    public double yMax3D = MarchingCubes.DEFAULT_Y_MAX;
    public double zMin3D = MarchingCubes.DEFAULT_Z_MIN;
    public double zMax3D = MarchingCubes.DEFAULT_Z_MAX;
    public int step3D = MarchingCubes.DEFAULT_GRID_SIZE;


    private static HashSet<String> texHashSet = texWordsToHashSet();

    private static HashSet<String> texWordsToHashSet() {
        // Standard TeX words used wich will not be surrounded with \mathbf{}.
        String[] words = {"times", "star", "ast", "sum", "prod", "int", "in",
                "over", "infty", "sqrt", "begin", "end", "hline", "cline", "\\",
                "le", "ge", "ne", "&", "lor", "neg",
                ...
                "longleftarrow", "longrightarrow", "downarrow", "uparrow", "longleftrightarrow", "coprod", "Leftrightarrow", "implies"
        };
    ...
    public CheckResult check(String usIn, String db) {
        return checkStrs(usIn, db) ? CheckResult.OK : CheckResult.WRONG;
    }
    ...
    public CheckResult check(List<String> userSolutionSections, List<String> dbSolutionSections) {
        ...
        F f_in = Parser.getF(us, this.ring, Collections.emptyList(), this.nonCommutWithIndices);
        F f_db = Parser.getF(db, this.ring, Collections.emptyList(), this.nonCommutWithIndices);
        F f = new F(F.SUBTRACT, f_db, f_in);
    ...
    boolean checkStrs(String usIn, String db) {
        LOG.info("User solutions: {}, \nSolutions from DB: {}", usIn, db);
        String us;
        int pos_sem = usIn.indexOf(';');
        int pos_eq = usIn.indexOf('=');
        us = (pos_sem > 0) ? usIn.substring(pos_eq + 1, pos_sem) : usIn.substring(pos_eq + 1);
        ...
        F f_in = Parser.getF(us, this.ring, Collections.emptyList(), this.nonCommutWithIndices);
        F f_db = Parser.getF(db, this.ring, Collections.emptyList(), this.nonCommutWithIndices);
        F ch = new F(F.SUBTRACT, f_db, f_in);
        ArrayList<Element> expression = new ArrayList<>();
        replaceOfSymbols(ch, expression);
        Element f_out = cf.InputForm(ch, this, expression);
        return f_out.isZero(ring);
    }
...
// ///////////////////////////////////////////////////////////////////////
    ...
    public String replaceOfSymbolsInIndeces(String obj, List<Element> expression) {
        ...
        if ((b != -1) && (bCirc != -1) && (obj.substring(b + 1, bCirc).trim().equals(""))) {
            return obj;
        }
     ...
     * the expression.
     *
     * @param obj        -- any kind of Elements, which may be puted to ezpression
     * @param expression previously obtained elements
     */
    public void replaceOfSymbols(Element obj, List<Element> expression) {
        if (expression.isEmpty()) {
            return;
        }
        while ((obj instanceof F) && (((F) obj).name == F.ID)) obj = ((F) obj).X[0];
        int len = 0;
        Element[] X = null;
        if (obj instanceof F) {
                ...
                if (objFname.indices.length > 0) {
                    ...
                    if (Rind != null) {
                        for (int j = 0; j < Rind.length; j++) {
                            if (Rind[j] != null) {
                                replaceOfSymbols(objFname.indices[0][j], expression);
                            }
                        }
                    }
                }
    ...
    public boolean contentNameInNCIE(String n) {
        Element pos = searchInMatricesElements(n);
        return pos != null;
    }
    ...
    public Object valueOf(Page page, int objPosition, Element[] values, Ring ring) {
        ...
        if (obj instanceof F) {
            ...
            return X1.value(values, ring);
        }
        return null;
    }


    String strToTexStr(String str) {
        return strToTexStrMain(str).replace("$", "").replace('\n', ' ');
    }
     ...
     */
    public String strToTexStrMain(String str) {
        ...
        /* 1 ----------------------------------------------------------------------------*/
        int pbs = str.indexOf('\\'); // Pointer to backslash in ST//
        int pb = 0;
        String word = "";
        String wordPrev;
        while (pbs != -1) {
            ...
            if ((pw == pbs + 1) && ((w == '&') || (w == '\\') || (w == '!')))
                pw++;  // ecpasialy for these 3 operators: \&,\\,\!

            wordPrev = word;
            word = str.substring(pbs + 1, pw); // служебное слово из букв и восклицательного знака иди \ или &
            a:
            {
                if (texHashSet.contains(word)) {
                    break a;
                }

                if (word.equals("hbox"))
                    if (wordPrev.equals("bf")) { // ------------------- "hbox +bf "
                        NT.append(str, pb, pbs);
                        NT.append("\\textbf{ ");
                        pbs += 5;
                        pb = pbs + 1;
                        break a;
                    } else break a;
                if (word.equals("binom")) {// ------------------- "binom"
                    NT.append(str, pb, pbs);
                    int pbs1 = str.indexOf('(', pbs + 1);
                    int para = FUtils.posOfPairBracket(str, pbs1, '(', ')');
                    String dd = str.substring(pbs1 + 1, para);
                    dd = dd.replace(",", "}{");
                ...
                if (word.equals("log")) { // ------------------- "log"
                    int pbsU = str.indexOf("_{", pbs + 1);
                ...
                if (word.equals("rootOf")) { // ------------------- "rootOf"
                    ...
                    NT.append("\\sqrt[").append(ppow)
                            .append("]{").append(dd, 0, iind).append("}");
                    pb = para + 1;
                    pbs = para;
                    break a;
                }
                if (word.equals("degreeC")) { // ------------------- "degreeC"
                    NT.append(str, pb, pbs);
                ...
                if (word.equals("degreeK")) {// ------------------- "degreeK"
                    NT.append(str, pb, pbs);
                ...
                if ((word.equals("d")) || (word.equals("D"))) {// ------------------- "d,D"
                    NT.append(str, pb, pbs);
                    int tempPbs = str.indexOf('(', pbs + 1);
                    if (tempPbs == -1) {
                        pb = pbs + 2;
                        NT.append(str, pbs, pb);
                        pbs++;
                        break a;
                    }
                    pbs = tempPbs;
                    int posUnder = str.lastIndexOf('_', pbs);
                    if (posUnder < pb) posUnder = -1;
                    String ind = (posUnder == -1) ? "" : str.substring(posUnder, pbs);
                    int para = FUtils.posOfPairBracket(str, pbs, '(', ')');
                    String dd = str.substring(pbs + 1, para);
                ...
                if (word.equals("ic")) {   // ------------------- "ic"
                    NT.append(str, pb, pbs);
                    pbs = str.indexOf('(', pbs + 1);
                    int para = FUtils.posOfPairBracket(str, pbs, '(', ')');
                    String dd = str.substring(pbs + 1, para);
                ...
                if (word.equals("elementOf")) {   // ------------------- "elementOf"
                    String str1 = str.substring(pb, pbs);
                    ...
                    if (elem == -1) {
                        ...
                        NT.append(str1, 0, elem + 1);
                    }
                    elemName = elemName.replace('=', ' ').trim();
                    int pbs1 = str.indexOf('(', pbs + 1);
                    int para = FUtils.posOfPairBracket(str, pbs1, '(', ')');
                    String dd = str.substring(pbs1 + 1, para).trim();
...
//                    }// теперь Matrix==true, если матрица нашлась
                ...
                if (word.equals("fact") || word.equals("!")) {// ------------------- "fact, !"
                    NT.append(str, pb, pbs);
                    int pbs1 = str.indexOf('(', pbs + 1);
                    int para = FUtils.posOfPairBracket(str, pbs1, '(', ')');
                    String dd = str.substring(pbs1 + 1, para);
                ...
                if (word.equals("cubrt")) {// ------------------- "cubrt"
                    NT.append(str, pb, pbs).append("\\sqrt[3]");
                    pb = pw;
                    break a;
                }
                if (word.equals("series")) { // ------------------- "series"
                    NT.append(str, pb, pbs).append("\\sum_");
                    pb = pw;
                    break a;
                }                            // ------------------- "system,   systLDE............"
                if (word.equals("systLDE") || (word.equals("initCond")) || (word.equals("system")) || (word.equals("equation")) || (word.equals("systemOR"))) {
                    NT.append(str, pb, pbs); // + "\\sum_"; pb = pw;break a;
                    pbs = str.indexOf('(', pbs + 1);
                    int para = FUtils.posOfPairBracket(str, pbs, '(', ')');
                    String dd = str.substring(pbs + 1, para);
                    dd = strToTexStr(dd);
                    String[] SS = FUtils.cutByCommas(dd, ring);
                    StringBuilder SSnew = new StringBuilder((SS.length == 1) ? "\\begin{array}{rcl}" :
                    ...
                    for (int j = 0; j < SS.length; j++) {
                        ...
                        if (equ != -1) {
                            SSnew.append(SS[j], 0, equ).append(compSig[i]).append(SS[j].substring(equ + shiftS));
                            if (j < SS.length - 1) SSnew.append(" \\\\ ");
                        }
                    }
                ...
                NT.append(str, pb, pbs);   // ----------------------------- окружаем \bf
                char cccc = str.charAt(pbs + 1); //слэш сос словом окружаем mathbf{}
        ...
        NT.append(str.substring(pb)); // Have changed all words with backslash
        ...
        /* 3 -----------------------------------------------------------------------*/
        int i = 0, j = 0, l, m, k, k2, kk2 = 0, z, prev = 0;
        StringBuilder N = new StringBuilder();
        StringBuilder M = new StringBuilder();
        String MM;
        StringBuilder C;
        a:
        while (true) {                                // Пересчет матрицы
            int iTmp, jTmp;
            while (true) {
            ...
            do {
                C = new StringBuilder("c");        // C -- Number of columns in matrix   //
                k = Matr.indexOf("]", m);
                z = Matr.indexOf(",", m);
        ...
        N.append(T.substring(prev, T.length()));
        ...
        int kaw = N.toString().indexOf('"'); // позиция кавычек старшая: (kaw>kaw0)
        boolean inText = (kaw == 0);   // признак того, что мы находимся в тексте (иначе мы в операторах)
        if (kaw == 0) {
            kaw0 = kaw + 1;
            kaw = N.toString().indexOf('"', 1);
        }
        while (true) {
     ...
     */
    private int endOfOperator(String text, int beg) {
        StringBuilder str = new StringBuilder(text);
        if (text.indexOf('{', beg) > text.indexOf('}', beg)) {
            return -1;
        } else {
            return FUtils.posOfPareBracket(str, text.indexOf('{', beg), '{', '}');
        }
    }
     ...
     */
    private static boolean blankTeXString(StringBuilder s, int beg, int end) {
        int i = beg + 1;
        while (i < end) {
            char sh = s.charAt(i++);
            if ((sh != ' ') && (sh != '\t') && (sh != '\n')) {
    ...
    private String deleteComments(String ss) {
        ...
        StringBuilder res = new StringBuilder(ss.substring(0, kaw0));
        int kaw = ss.indexOf('"', kaw0 + 1);
        int pTASK = ss.lastIndexOf("TASK", kaw);
        if (pTASK >= 0) {
            expr.clear();
            ring.CForm.vectEl[Ring.Fname] = new Vector<>();
            nonCommutWithIndices.clear();
            ring.CForm.vectF[0] = new Vector<>();
        ...
        while (kaw != -1) {
            res.append((inText) ? ";" : ss.substring(kaw0, kaw));
            inText = !inText;
            kaw0 = kaw + 1;
            kaw = ss.indexOf('"', kaw0);
        }
        if (!inText) {
            res.append(ss.substring(kaw0));
        }
        return res.toString();
    }
     ...
     * backslash+blank
     */
    public static void addTexBlanks(StringBuilder nn) {
        int ll = nn.indexOf("  ");
        while (ll != -1) {
            ll++;
            nn.insert(ll, '\\');
     ...
     */
    private void insertObj(String name, Element ob, List<Element> expression) {
     ...
     */
    private void saveFuncs(String name, Element ob, IntList proc) {
...
//    }
     ...
     */
    private void readSingleOperator(String expres, IntList proc) {
...
//        } // system was excluded
        ...
        Element fOut;  // Здесь будет объект после обработки парсером правой части
        switch (objName) {
            case "SPACE":
                try {
                    setRing(rhsExpr);
            ...
            case "MOD":
                ...
                break;
            case "SMALLESTBLOCK": // SMALLEST MATRIX BLOCK for recursive matrix multiplication
                try {
            ...
            default:
                ...
                if (severEq < -1) {
                    ring.exception
                            .append("Syntax error. Bracket (  ")
                            .append((severEq == -2) ? '{' : (severEq == -3) ? '(' : '[')
                            .append(" not closed ");
                }
                ...
                if (left.name == F.ID && left.X[0] instanceof Fname) {
                    ...
                    pol_case:
                    // разрешаем иметь "имена" в левой части, которые совпадают с переменными из кольца
                    {
     ...
     */
    private int getIndexElementByNameFromExpr(String name, List<Element> expr1) {
     ...
     */
    private Element getElementByNameFromExpr(String name) {
        for (int i = expr.size() - 1; i >= 0; i--) {
            if (expr.get(i) instanceof Fname) {
                Fname el = ((Fname) expr.get(i));
                if (el.name.equals(name)) {
                    if ((el.X != null) && (el.X[0] != null))
                        return el.X[0];
                    else return null;
                }
            }
    ...
    private Element go(int procNum, List<Element> expression) {
        ...
        while (i < currTransProg.length) {
            if (currTransProg[i] >= 0) {
                ...
                if (!trySetConstants(fn, currTransProg, i)) {
                    ...
                    switch (f_name) {
                        ...
                        case F.SET3D:
                            this.xMin3D = ((Polynom) f.X[0]).coeffs[0].value;
                            this.xMax3D = ((Polynom) f.X[1]).coeffs[0].value;
                            this.yMin3D = ((Polynom) f.X[2]).coeffs[0].value;
                            this.yMax3D = ((Polynom) f.X[3]).coeffs[0].value;
                            this.zMin3D = ((Polynom) f.X[4]).coeffs[0].value;
                            this.zMax3D = ((Polynom) f.X[5]).coeffs[0].value;
                            if (f.X.length == 7) {
                                this.step3D = (int) ((Polynom) f.X[6]).coeffs[0].value;
                            }
                            break;
                        ...
                        case F.ELEMENTOF: //привязываем к нашему объекту через ссылку
                            ...
                            if (pare[0] == null) {
                                cfPage.RING.exception.append(" Not correct operator <<\\elementOf()>>.");
                                return null;
                            }
                            Fname tempEOf = ((Fname) pare[0]);
                            tempEOf.X = new Element[]{pare[1]};
                            //  Element tempEOf = expandFnameOrId(f.X[0]);
                            System.out.println("tempEOf==" + tempEOf);
                            replaceOfSymbols(tempEOf, expression);
                            int posInExpr = getIndexElementByNameFromExpr(tempEOf.name, expression);
...
//                                } else {
                            ...
                            f.showGraphics(this, true, false, false, namesFunc.toArray(new String[0]));
                            insertObj(((Fname) data.funcs.get(currTransProg[i])).name, f, expression);
                            printLastExpr = false;
                            break;
                        }
                        ...
                        case F.SHOW_PLOTS_3D:
                            ArrayList<String> nameFuncs = new ArrayList<>();
                            for (int index = 0; index < ((F) f.X[0]).X.length; index++) {
                                if (((F) f.X[0]).X[index] instanceof F) {
                                    if (((F) ((F) f.X[0]).X[index]).name == F.TABLEPLOT) {
                                        nameFuncs.add(String.valueOf(index));
                                    } else {
                                        nameFuncs.add(((F) f.X[0]).X[index].toString(ring));
                                    }
                                } else {
                                    nameFuncs.add(((F) f.X[0]).X[index].toString(ring));
                                }
                            ...
                            plots.put(currentSectionNumber, f);
                            replaceOfSymbols(f, expression);
                            f.showGraphics(this, true, false, false, nameFuncs.toArray(new String[nameFuncs.size()]));
                            insertObj(((Fname) data.funcs.get(currTransProg[i])).name, f, expression);
                            printLastExpr = false;
                            break;
//                            replaceOfSymbols(f, expression);
...
//                            break;
                        ...
                        case F.PAINTELEMENT: {
                            ...
                            String input = ((Fname) ((F) ((F) f.X[0]).X[0]).X[0]).name;
                            ...
                            if (parts[0].equals("Circle")) {
                                ...
                                try {
                                    ...
                                    execution("ERROR. Couldn't parse the Circle radius", procNum);
                                    break;
                                }
                                ...
                                // Get parser results
                                execution(circle.draw(), procNum);
                            } else {
                                execution("ERROR. Input was not handled", procNum);
                            }
...
//                                }
//                            }
//                            // "to png-file":  f.showGraphics(this, false, true, false, names.toArray(new String[names.size()]));
//                            f.showGraphics(this, true, true, false, names.toArray(new String[0]));
//                            insertObj(((Fname) data.funcs.get(currTransProg[i])).name, f, expression);
//                            printLastExpr = false;
                        }
                        case F.SOLVEPDE:
                ...
                switch (currTransProg[i]) {
                    ...
                    case OP_RETURN: {
                        F f = new F(((Fname) data.funcs.get(currTransProg[i + 1])).X[currTransProg[i + 2]]);
                        replaceOfSymbols(f, expression);
                        f_out = cfPage.InputForm(f, this, expression);
        ...
        if (printLastExpr && procNum == data.transProg.size() - 1) {
            ...
            if (!ring.STEPBYSTEP.isZero(ring) && !ring.traceSteps.isEmpty()) {
                ...
                if ((obj instanceof Fname) && (((Fname) obj).X != null) && (((Fname) obj).X[0] != null)
                    ...
                    sb.append("\n").append(((Fname) obj).name).append("=").append(obj.toString(ring));
    ...
    public Element runProcedure(F inputProcedure, List<Element> lastExpress) {
        ...
        List<Element> express = new ArrayList<>(16);
            ...
            while (j <= args.length()) {
                if (j > 0) { // цикл для всех аргументов, кроме последнего
                    ...
                    if (fOut instanceof Fname) {
                        ...
                        fOut = new Fname(fOutName.substring(2));
        ...
        for (int ii = 0; ii < procArgs.length; ii++) {
            if (procArgs[ii] instanceof Fname) {
                ...
                String nname = procArgName.substring(2);
    ...
    private int procedureCaught(String str) {
        int i = str.indexOf("(");
    ...
    private void readOperators(String str, IntList proc) {
        ...
        while (notLastOperator) {
            ...
            if ((this.ring.TIMEOUT == 15) && (expr1.length() > 160)) {
                expr1 = "VeryLongExpression";
                this.ring.exception.append("Operator has more than 160 letters. You may change timeout. Do operator: TIMEOUT=16;  )");
            }
            operatorsCount++;
            if (!expr1.equals("")) {
                /*=============================================*/
                if (expr1.indexOf("if") == 0) {
                    ...
                    opEnd = opBegin + procedureCaught(str.substring(opBegin));
                    /*=============================================*/
     ...
     */
    private String getArgElementOf(Element e) {
        if (e instanceof F) {
     ...
     */
    private Element[] expandFnameOrId2(Element[] g) {
        if (g[1] instanceof Fname) {
            g[0] = g[1];
            if (((Fname) (g[1])).isEmpty(ring)) {
                g[1] = null;
                return g;
            } else {
                Element W = ((Fname) (g[1])).X[0];
                expandFnameOrId(W);
                g[1] = W;
                return g;
            }
    ...
    private void handleReplot(F f, List<Element> expression, CanonicForms cfPage) {
        ...
        if (func.name == F.TABLEPLOT) {
            if (fInput.X[0] instanceof MatrixD) {
                ...
                fInput.X[0] = t;
                plots.put(currentSectionNumber, fInput);
            } else if (fInput.X[0] instanceof VectorS) {
                MatrixD ss = new MatrixD(((VectorS) fInput.X[0]));
                t = new Table(ss);
                //
                fInput.X[0] = t;
                plots.put(currentSectionNumber, fInput);
            ...
            if (func.name == F.POINTSPLOT || func.name == F.PLOT || func.name == F.PARAMPLOT || func.name == F.PLOT3D) {
                ...
                List<String> namesFunc = new ArrayList<>();
                for (int index = 0; index < ((F) func.X[0]).X.length; index++) {
                    namesFunc.add("f" + index);
                }
                func.showPlots(this, paramsAndSettings, true, namesFunc.toArray(new String[0]));
            }
        }
    }
    ...
    private void callplot(F func, Element[] arr) {
        switch (func.name) {
            ...
            case F.PLOT3D:
                Element[] vals = new Element[ring.varPolynom.length];
                System.arraycopy(ring.varPolynom, 0, vals, 0, 2);
                if (ring.varPolynom.length - 2 >= 0) {
                    System.arraycopy(arr, 0, vals, 2, ring.varPolynom.length - 2);
                }
     ...
     * @param t  - заголовок
     */
    private void set2D_old(Element x0, Element x1, Element y0, Element y1, Element x, Element y, Element t) {
    ...
    private int getFramesNumber(int section) {
    ...
    private void xyzCube(double[][] cube) {
...
//        int closeQuoteIdx = rhs.indexOf('\'', openQuoteIdx + 1);
     ...
     * (like "FLOATPOS = ...;").
     */
    private boolean trySetConstants(Fname fn, int[] mass, int i) {
