...
//    }
    ...
    public Fraction clone() {
        ...
        if(max < Ring.Polynom) {
            ...
            return new Fraction((Element) num.clone(), (Element) denom.clone());
        }
    }
...
//                .divideToFraction(new Polynom(polDenom.powers,new Element[]{ring.numberONE}),ring) ;
        ...
        if (denomM.isOne(ring)) { return numM;}
        if (denomM instanceof Fraction) {
            return new Fraction(numM.multiply(((Fraction) denomM).denom, ring), ((Fraction) denomM).num);
        }
        return new Fraction(numM, denomM);
    }
    ...
    public Element pow(int n, Ring ring) {
        ...
        if (Res instanceof Fraction) {
            return new Fraction(((Fraction) Res).num.pow(n, ring), ((Fraction) Res).denom.pow(n, ring)).cancel(ring);
        } else {
 ...
 */
    ...
    /** Value of sin from number (in radians) equals this fraction multiplied by \pi
     *  We try to obtain the rational or algebraic answer
     * @param ring
     * @return sin(\pi*this)
     */    
    @Override
    ...
    /** Value of cos from number (in radians) equals this fraction multiplied by \pi
     *  We try to obtain the rational or algebraic answer
     * @param ring
     * @return cos(\pi*this)
     */
    ...
    /** Value of tan from number (in radians) equals this fraction multiplied by \pi
     *  We try to obtain the rational or algebraic answer
     * @param ring
     * @return tan(\pi*this)
     */
    ...
    /** Value of ctg from number (in radians) equals this fraction multiplied by \pi
     *  We try to obtain the rational or algebraic answer
     * @param ring
     * @return ctg(\pi*this)
     */
