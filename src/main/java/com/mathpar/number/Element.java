...
import com.mathpar.lcTransform.specFuncs;
...
public class Element implements Serializable, Comparable<Element>, Cloneable {
    ...
    public Element subtract(Element x, Ring ring) {
        if (this == NAN || x == NAN) { return NAN; }
        if (x.getClass() != this.getClass()) { return NAN;} 
...
//    }   
    ...
    public Element LCM(Element x, Ring ring) {
        if(x instanceof Polynom){return x.LCM(this, ring);}
   ...
   /**
    ...
    public Element Mod(Ring r) {
      ...
      //  r.exception.append("Error: modInverse exists only for NumberZ tipe of numbers. "   );
        return null;
    } // a* a.modInverse(m) = 1 mod m   FOR NumberZ
...
/**
    ...
    public Element pow(int n, Ring r) {int m=(n<0)?-n:n;
       ...
       if ((this instanceof Polynom)&&((Polynom)this).coeffs.length==1){Polynom pp=((Polynom)this);
             int[] npow=new int[pp.powers.length];  Element[] cof=new Element[]{pp.coeffs[0].pow(m,r)};
    ...
    */
    ...
    public Element gamma(Ring r) {
        return new NumberR64(new specFuncs().gamma(doubleValue()));
    }

    public Element beta(Element a, Ring r) {
        return new NumberR64(new specFuncs().beta(doubleValue(), a.doubleValue()));
    }
...
/**
    ...
    */
    ...
    public Element B_NOT(Ring ring) {
      if ( isZero(ring)) return TRUE;
      if ( isOne(ring))   return FALSE ;
    ...
    public Element B_AND(Element B, Ring ring) {
        if ( B.isItNumber(ring)  && this.isItNumber(ring))
           return  ((this.isOne(ring))&&(B.isOne(ring)))?  TRUE: FALSE;
    ...
    public Element B_OR(Element B, Ring ring) {
        if ( B.isItNumber(ring)  && this.isItNumber(ring))
           return ((this.isZero(ring)) &&(B.isZero(ring)))? FALSE :  TRUE;
     ...
     * @return 
     */
    public Element B_IMPLIES(Element B, Ring ring) {
        if ( B.isItNumber(ring)  && this.isItNumber(ring))
           return (this.isZero(ring))?   TRUE: (B.isOne(ring))?  TRUE: FALSE;
        return new F(F.B_IMPLIES, new Element[]{this,B});
    }
        /**
     * Logical EQUIVALENS
     * @param B
     ...
     * @return 
     */
        public Element B_EQUIV(Element B, Ring ring) {
         if ( B.isItNumber(ring)  && this.isItNumber(ring))
           return ((this.isZero(ring))&&(B.isZero(ring)) || (this.isOne(ring))&&(B.isOne(ring)))?  TRUE : FALSE;
        return new F(F.B_EQUIV, new Element[]{this,B});
    }
