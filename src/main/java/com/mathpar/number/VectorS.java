...
public class VectorS extends Element  {
    ...
    public Element multiply(Element x, Ring ring) {int flX=0; int resFL=0;
        ...
        if (x instanceof VectorS){VectorS S=((VectorS)x); flX=S.fl;
            resFL= (flX!=0)? 1: (fl==-1)? 0:1;
            if ((flX>=0)&&(fl<0)){ return new MatrixD(multiplyOuter(S.V, ring), resFL);}
            else if ((flX<0)&&(fl>=0)){ return  multiply(S.V, ring);}
            else if ( S.V[0] instanceof VectorS) {MatrixD md=new MatrixD((VectorS)this); 
            Element tt=( S.V[0] instanceof VectorS)?  new MatrixD((VectorS)S):S;
            return md.multiply(tt, ring);}}
 ...
 */
    ...
    public Element B_NOT(Ring ring) {
         Element[] nV= new Element[V.length];
        Element ComDen = NumberZ.ONE;
        for (int j = 0; j < V.length; j++)   
          nV[j]=V[j].B_NOT(ring);
        return new VectorS(nV);
    ...
    public Element B_AND(Element B, Ring ring) {
        if(B instanceof VectorS) return B_AND((VectorS)B, ring);
        else return (V.length==0)? new VectorS(new Element[]{B}):
                     new VectorS(new Element[]{V[0].B_AND(B, ring)});
    }
    /**
     *  Logical AND
     * @param B
     ...
     * @return 
     */
    public Element B_AND(VectorS B, Ring ring) {
        if(V.length>B.V.length){ring.exception.append
              ("Different length of VectorS in boolean AND. ");
         return null;}
         Element[] nV= new Element[V.length];
        for (int j = 0; j < V.length; j++) nV[j]=V[j].B_AND( B.V[j], ring);
        return new VectorS(nV);
    }
    /**
     * Logical OR
     ...
     * @return 
     */
    public Element B_OR(VectorS B, Ring ring) {
        if(V.length>B.V.length){ring.exception.append
              ("Different length of VectorS in boolean OR. ");
         return null;}
         Element[] nV= new Element[V.length];
        for (int j = 0; j < V.length; j++) nV[j]=V[j].B_OR( B.V[j], ring);
        return new VectorS(nV);
    }
    /**
     * Logical OR
    ...
    public Element B_OR(Element B, Ring ring) {
        if(B instanceof VectorS) return B_OR((VectorS)B, ring);
        else return (V.length==0)? new VectorS(new Element[]{B}):
                     new VectorS(new Element[]{V[0].B_OR(B, ring)});
    }
    
 
 
    /**
     * Добавляет элемент в вектор на последнее место
     * @param add добавляемый элемент
     * @return [this[0], this[1] ..., add]
     */
    public VectorS addElement(Element add){
        Element[] res = new Element[this.V.length+1];
     ...
     * @param last вектор, с которым нужно объединить исходный
     * @return [элементы первого вектора, элементы второго вектора]
     */
    public VectorS merge(VectorS last){
        if(this.length() == 0) return last;
        if(last.length() == 0) return this;
        Element[] res = new Element[this.V.length+last.V.length];
        for (int i = 0; i < res.length; i++)
            if(i < this.V.length) res[i] = this.V[i];
            else res[i] = last.V[i-this.V.length];
        return new VectorS(res);
    }
    
    /**
     * Проверяет существование элемента или противоположного ему в векторе
     * @param el
     * @param ring
     * @return
     */
    public boolean exist(Element el, Ring ring){
        Element neg = el.negate(ring);
        for (int i = 0; i < this.V.length; i++) {
            if(this.V[i].subtract(el, ring).isZero(ring)) return true;
            if(this.V[i].subtract(neg, ring).isZero(ring)) return true;
        }
        return false;
}}
