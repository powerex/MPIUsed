 ...
 */
    ...
    public static Element[] concatenation(Element[]a, Element[]b){
        int n=a.length+b.length; Element[] c=new Element[n];
        System.arraycopy(a, 0, c, 0, a.length);
        System.arraycopy(b, 0, c, a.length, b.length);
        return c;
    }   
        public static int[] concatenation(int[]a, int[]b){
        int n=a.length+b.length; int[] c = new int[n];
        System.arraycopy(a, 0, c, 0, a.length);
        System.arraycopy(b, 0, c, a.length, b.length);
        return c;
    }
...
/**
 * Get next combination from N by k
 * @param prev previous combination. Start from [0,1,2,..,k-1]
 * @param Nmin1 = N-1 (combination from N elements by k)
 * @return int[k] with new combination or null (if no more)
 */
public static int[] nextCombination (int[] prev, int Nmin1) {
    int k=prev.length; int j=0; int p=k-1;  int[] res=new int[k];
    while((k>j)&&(prev[p-j]==Nmin1-j))j++;
    if (j>=k) return null; //System.out.println("k,j=="+k+j);
    res[p-j]=prev[p-j]+1;
    for (int i=p-j+1 ; i < k; i++) res[i]=res[i-1]+1;
    System.arraycopy(prev,0, res, 0, p-j);
    return res;
    }
   /**
 * Get next combination from N by k
 ...
 * @return int[k] with new combination or null (if no more)
 */
public static int[][] allCombinationsNbyK (int N, int K) {
    NumberZ Cnk=(NumberZ)NumberZ.binomial(Ring.ringZxyz, N, K);
     System.out.println("cnk="+Cnk);
    long cnkL= (long)Cnk.toNumber(Ring.Z64, Ring.ringZxyz).value;
    System.out.println("cnk="+cnkL);
    int[][] res=new int[(int)cnkL][0];
    int[] prev=new int[K]; 
    for (int i = 0; i < K; i++) {prev[i]=i;
    } int j=0;   
     while (prev!=null){   res[j++]=prev;
     prev=nextCombination (prev, N-1);}
    return res;
    }
...
//        int[] p=null; //new int[]{10,50,100};
  ...
  int[][] rr=allCombinationsNbyK (4, 2);
             System.out.println( ""+ Array.toString(rr));
         }
